...
Windows uses Windows Graphics Capture, the WGC hardware encoder, WASAPI loopback, CPAL and Win32 cursor sampling. macOS uses ScreenCaptureKit, its VideoToolbox-backed recording output, a separate ScreenCaptureKit audio stream, CPAL and CoreGraphics cursor events. Linux uses the XDG ScreenCast portal plus PipeWire, with X11/XFixes as an isolated fallback. Webcams are captured by Chromium with `getUserMedia`/`MediaRecorder`; Electron atomically persists their WebM segments beside native tracks. FFmpeg, when used for Linux H.264, is an optional child process and does not appear in public types.
