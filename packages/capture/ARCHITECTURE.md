...
Windows uses Windows Graphics Capture, the WGC hardware encoder, WASAPI loopback, CPAL, `cameras`/Media Foundation and Win32 cursor sampling. macOS uses ScreenCaptureKit, its VideoToolbox-backed recording output, a separate ScreenCaptureKit audio stream, CPAL/`cameras` AVFoundation discovery and CoreGraphics cursor events. Linux uses the XDG ScreenCast portal plus PipeWire, with X11/XFixes as an isolated fallback; local webcams are discovered through `cameras`/V4L2. FFmpeg, when used for Linux H.264, is an optional child process and does not appear in public types.
