...
function microphoneFormat(value = UNKNOWN_OPUS_FORMAT) {
  ...
  for (const [key, name] of [['sampleRate', 'Microphone sample rate'], ['channels', 'Microphone channels']]) if (!Number.isSafeInteger(value[key]) || value[key] < 0) throw new Error(`${name} must be a non-negative integer.`)
...
function createMicrophoneStorage(options) {
  return createMediaSegmentStorage({ kind: 'microphone', sourcePrefix: 'microphone:chromium:', normalizeFormat: microphoneFormat, ...options })
}
