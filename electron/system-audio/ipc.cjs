...
function systemAudioFormat(value = UNKNOWN_OPUS_FORMAT) {
  ...
  for (const [key, name] of [['sampleRate', 'System audio sample rate'], ['channels', 'System audio channels']]) if (!Number.isSafeInteger(value[key]) || value[key] < 0) throw new Error(`${name} must be a non-negative integer.`)
...
function createSystemAudioStorage(options) {
  return createMediaSegmentStorage({ kind: 'system-audio', manifestKey: 'systemAudio', sourcePrefix: 'system-audio:chromium:', normalizeFormat: systemAudioFormat, ...options })
}
