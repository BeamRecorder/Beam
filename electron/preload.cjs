...
const invoke = (command, payload) =>
  ipcRenderer.invoke("capture:request", command, payload);
...
contextBridge.exposeInMainWorld(
  ...
  Object.freeze({
    ...
    start: (config) =>
      config ? invoke("start-recording", { config }) : invoke("start"),
    pause: () => invoke("pause"),
    resume: () => invoke("resume"),
    stop: () => invoke("stop"),
    status: () => invoke("status"),
    beginCameraSegment: (payload) =>
      ipcRenderer.invoke("camera:begin-segment", payload),
    writeCameraSegment: (payload) =>
      ipcRenderer.invoke("camera:write-segment", payload),
    finalizeCameraSegment: (payload) =>
      ipcRenderer.invoke("camera:finalize-segment", payload),
    failCamera: (payload) => ipcRenderer.invoke("camera:fail", payload),
    beginMicrophoneSegment: (payload) =>
      ipcRenderer.invoke("microphone:begin-segment", payload),
    writeMicrophoneSegment: (payload) =>
      ipcRenderer.invoke("microphone:write-segment", payload),
    finalizeMicrophoneSegment: (payload) =>
      ipcRenderer.invoke("microphone:finalize-segment", payload),
    failMicrophone: (payload) => ipcRenderer.invoke("microphone:fail", payload),
    beginSystemAudioSegment: (payload) =>
      ipcRenderer.invoke("system-audio:begin-segment", payload),
    writeSystemAudioSegment: (payload) =>
      ipcRenderer.invoke("system-audio:write-segment", payload),
    finalizeSystemAudioSegment: (payload) =>
      ipcRenderer.invoke("system-audio:finalize-segment", payload),
    failSystemAudio: (payload) =>
      ipcRenderer.invoke("system-audio:fail", payload),
    ...
    createProject: (options = {}) =>
      ipcRenderer.invoke("projects:create", options),
    renameProject: (projectId, name) =>
      ipcRenderer.invoke("projects:rename", { projectId, name }),
    saveProjectThumbnail: (projectId, dataUrl) =>
      ipcRenderer.invoke("projects:save-thumbnail", { projectId, dataUrl }),
    deleteProject: (projectId) =>
      ipcRenderer.invoke("projects:delete", { projectId }),
    getProjectComposition: (projectId) =>
      ipcRenderer.invoke("projects:composition", { projectId }),
    saveProjectComposition: (projectId, composition) =>
      ipcRenderer.invoke("projects:save-composition", {
        projectId,
        composition,
      }),
    pickProjectCompositionMedia: (projectId, kind) =>
      ipcRenderer.invoke("projects:pick-composition-media", {
        projectId,
        kind,
      }),
    ...
    configureCameraOverlay: (state) =>
      ipcRenderer.send("camera-overlay:configure", state),
    setCameraOverlayActive: (active) =>
      ipcRenderer.send("camera-overlay:set-active", Boolean(active)),
