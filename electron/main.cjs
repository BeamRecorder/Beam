const { app, BrowserWindow, desktopCapturer, ipcMain, session, protocol, net } = require('electron')
...
app.whenReady().then(() => {
  ...
  protocol.handle('whisper-model', (request) => { const file = whisperStore.fileForUrl(request.url); return file ? net.fetch(pathToFileURL(file).href) : new Response('Not found', { status: 404 }) })
