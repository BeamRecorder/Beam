...
const idleState = (version, status = 'idle') => ({ status, currentVersion: version, availableVersion: null, percent: null, message: null })
...
function createAutoUpdater({ app, BrowserWindow, autoUpdater, openExternal, isPackaged = app.isPackaged }) {
  ...
  autoUpdater.on('checking-for-update', () => setState({ status: 'checking', availableVersion: null, percent: null, message: null }))
  autoUpdater.on('update-available', (info) => setState({ status: 'available', availableVersion: versionOf(info), percent: null, message: null }))
  autoUpdater.on('update-not-available', () => setState({ status: 'not-available', availableVersion: null, percent: null, message: null }))
  autoUpdater.on('download-progress', (progress) => setState({ status: 'downloading', percent: Math.round(progress.percent) }))
  autoUpdater.on('update-downloaded', (info) => setState({ status: 'downloaded', availableVersion: versionOf(info), percent: 100, message: null }))
  autoUpdater.on('error', (error) => setState({ status: 'error', percent: null, message: errorMessage(error) }))
  return {
    async checkForUpdates() { try { await autoUpdater.checkForUpdates() } catch (error) { setState({ status: 'error', percent: null, message: errorMessage(error) }) }; return state },
    getState: () => state,
    async downloadUpdate() {
      if (state.status !== 'available') return false
      try { await autoUpdater.downloadUpdate(); return true } catch (error) { setState({ status: 'error', percent: null, message: errorMessage(error) }); return false }
    },
    quitAndInstall: () => { if (state.status !== 'downloaded') return false; autoUpdater.quitAndInstall(); return true },
    openChangelog,
  }
}
