function registerTeleprompterIpc({ ipcMain, teleprompterWindow, storage }) {
  ...
  ipcMain.on('teleprompter:set-session', (_event, context) =>
    teleprompterWindow.setSession(context === null ? null : context),
  );
  ipcMain.on('teleprompter:ready', () => teleprompterWindow.markRendererReady());
