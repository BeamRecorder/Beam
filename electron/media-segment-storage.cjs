...
function createMediaSegmentStorage({ kind, manifestKey = kind, sourcePrefix, normalizeFormat, fsModule = fs, pathModule = path }) {
  ...
  const configure = (session, payload) => {
    if (session.ownerId !== null && session.ownerId !== payload.ownerId) throw new Error(`${trackName} session belongs to another renderer.`)
    if (typeof payload.sourceId !== 'string' || !payload.sourceId.startsWith(sourcePrefix)) throw new Error(`Invalid Chromium ${kind} source.`)
  ...
  return {
    ...
    write(ownerId, payload = {}) {
      ...
      if (!Number.isSafeInteger(payload.sequence) || payload.sequence !== job.nextSequence) throw new Error(`Invalid ${kind} chunk sequence.`)
      const data = payload.data
      if (!(data instanceof Uint8Array) || data.byteLength === 0 || data.byteLength > MAX_CHUNK_BYTES) throw new Error(`Invalid ${kind} chunk size.`)
    ...
    finalize(ownerId, payload = {}) {
      ...
      jobs.delete(job.id)
      session.segments.push({ segmentId: crypto.randomUUID(), path: `${kind}/${pathModule.basename(job.targetPath)}`, startNs: job.startNs, endNs, complete: true })
    ...
    fail(ownerId, payload = {}) {
      ...
      else if (session.ownerId !== null && session.ownerId !== ownerId) throw new Error(`${trackName} session belongs to another renderer.`)
      if (typeof payload.reason !== 'string' || !payload.reason.trim()) throw new Error(`${trackName} failure reason is required.`)
