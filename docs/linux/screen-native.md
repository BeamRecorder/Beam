...
RPM et DEB installent le binaire root-owned dans `/usr/libexec/beam-input-helper` et sa policy Polkit dans `/usr/share/polkit-1/actions`. Une AppImage affiche une demande Polkit explicite depuis les preferences et installe exactement ces deux fichiers dans des chemins fixes. Cette action accorde durablement a l'utilisateur local actif le lancement de la seule commande `stream` du helper installe ; les lancements suivants de Beam ne redemandent donc pas le mot de passe. Une nouvelle autorisation reste necessaire pour installer ou mettre a jour le binaire et sa policy. Aucun paquet Flatpak, groupe `input`, regle udev permissive ni commande terminal n'est necessaire.

L'autorisation est disponible uniquement dans `HUD > Preferences > Enregistrer les raccourcis clavier`. Le HUD principal n'affiche aucune demande et ne change pas de taille. Apres cette action explicite, Beam reactive silencieusement le helper aux lancements suivants tant que le reglage reste active. Sous Linux, desactiver ce reglage coupe a la fois les raccourcis et les metadonnees de clics et empeche le demarrage automatique du helper au prochain lancement. Le sidecar commun est `cursor/input.json`. Les clics sont aussi fusionnes dans `cursor/cursor.json` avec la derniere position PipeWire connue afin de conserver les effets de clic et le zoom automatique existants.
...
- conserver les changements de l'ID SPA opaque lorsqu'ils sont significatifs ; sur Mutter, qui conserve souvent `MetaCursor.id = 1`, deriver aussi un identifiant opaque stable des pixels lorsqu'un bitmap valide est republié, puis jeter immediatement ces pixels sans les stocker ni les graver dans la video ;
