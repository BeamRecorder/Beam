...
The Crop Bar has two rows: labeled mode/preset/format fields, then grouped audio, Studio effects, and recording actions. Use native selects and translated native `title` hints so menus and hints are not clipped by the transparent surface. Clicking a field's icon or label focuses its select and opens its native picker; direct select clicks and keyboard operation retain native behavior. Hide Studio-only controls in Raw mode. OFF toggles use red crossed-out icons, separately from disabled controls. The controls container is `no-drag`; its full-height grip is a separate sibling so the interactive rectangle cannot subtract the drag handle.
