...
const preferences = (theme: PreferenceSettings['theme']): PreferenceSettings => ({
  schemaVersion: 2, theme, recordingBar: { visibility: 'always' }, devices: {}, shortcuts: {}, backgroundPresets: { colors: [], gradients: [] }, extras: {},
})

const capture = { getPreferences: vi.fn(), updatePreferences: vi.fn(), onPreferencesChanged: vi.fn() }
...
beforeEach(() => {
  ...
  window.matchMedia = vi.fn(() => ({ get matches() { return mediaMatches }, addEventListener: vi.fn((_type, listener) => { mediaChange = listener }), removeEventListener: vi.fn() }) as unknown as MediaQueryList)
...
afterEach(() => { delete window.capture })
...
describe('theme store', () => {
  ...
  it('synchronizes a preference change from another Electron renderer', async () => {
    ...
    capture.onPreferencesChanged.mockImplementation((callback: (value: PreferenceSettings) => void) => { notify = callback; return vi.fn() })
    const store = await loadStore()
