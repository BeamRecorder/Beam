...
describe("BrowserMicrophoneRecorder", () => {
  it("requests a processed stream, writes data, and finalizes a segment", async () => {
    ...
    expect(capture.beginMicrophoneSegment).toHaveBeenCalledWith(expect.objectContaining({ sessionId: "session-1", sourceId: "microphone:chromium:usb", startNs: 0 }));
    expect(capture.writeMicrophoneSegment).toHaveBeenCalledWith({ jobId: "mic-job-1", sequence: 0, data: new Uint8Array([4, 5]) });
    expect(capture.finalizeMicrophoneSegment).toHaveBeenCalledWith(expect.objectContaining({ jobId: "mic-job-1", endNs: 0, metrics: {} }));
  ...
  it("rejects unsupported setup and releases tracks after audio graph failures", async () => {
    ...
    await expect(BrowserMicrophoneRecorder.request("microphone:chromium:default")).rejects.toThrow("did not provide an audio track");
