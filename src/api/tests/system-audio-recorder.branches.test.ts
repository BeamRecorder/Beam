...
describe('BrowserSystemAudioRecorder', () => {
  ...
  it('persists failure when finalization itself fails', async () => {
    ...
    expect(capture.failSystemAudio).toHaveBeenCalledWith(expect.objectContaining({
      sessionId: 'session-4',
  ...
  it('rejects operations outside Electron and records standalone failures', async () => {
    ...
    await expect(recordSystemAudioFailure('session-6', 'permission denied')).rejects.toThrow('unavailable outside Electron');
  });
});
