...
beforeEach(() => {
  ...
  Object.defineProperty(navigator, 'mediaDevices', {
    configurable: true,
    value: { enumerateDevices: vi.fn(), getUserMedia: vi.fn() },
  });
...
describe('BrowserCameraRecorder', () => {
  it.each(['', 'camera:chromium:', 'camera:wrong:device', 'microphone:chromium:camera'])(
    'rejects malformed camera source ids: %s',
    async (sourceId) => {
  ...
  it('uses positive rounded negotiated settings and starts a persisted segment', async () => {
    ...
    expect(capture.beginCameraSegment).toHaveBeenCalledWith({
      sessionId: 'session',
      sourceId: 'camera:chromium:device-7',
      format: expect.objectContaining({
        width: 1280,
        height: 1080,
        nominalFps: 30,
        appearance: { shadowSize: 'md', cornerRadius: 'lg' },
      }),
      startNs: 0,
    });
  ...
  it('serializes chunks and finalizes using a non-negative monotonically bounded end time', async () => {
    ...
    nativeRecorder.emit('dataavailable', {
      data: { size: 5, arrayBuffer: async () => new Uint8Array([1, 2, 3]).buffer },
    });
    nativeRecorder.emit('dataavailable', { data: { size: 0, arrayBuffer: async () => new ArrayBuffer(0) } });
    await recorder.stop(123_000_000);
    expect(capture.writeCameraSegment).toHaveBeenCalledWith(
      expect.objectContaining({ jobId: 'job-1', sequence: 0, data: expect.any(Uint8Array) }),
    );
    expect(capture.finalizeCameraSegment).toHaveBeenCalledWith(
      expect.objectContaining({
        jobId: 'job-1',
        endNs: 123_000_000,
        metrics: { framesAcquired: 0, framesReceived: 0 },
      }),
    );
  ...
  it('starts a new segment after pause and keeps segment sequence numbers isolated', async () => {
    ...
    expect(capture.beginCameraSegment).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({ sessionId: 'session', startNs: expect.any(Number) }),
    );
