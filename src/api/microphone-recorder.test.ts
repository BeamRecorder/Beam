...
import { listBrowserMicrophones, microphoneDeviceId, normalizedMicrophoneSetting } from './microphone-recorder'
...
describe('microphone recorder helpers', () => {
  ...
  it.each(['', 'microphone:1', 'camera:chromium:device'])('rejects an invalid microphone source id', (sourceId) => {
    expect(() => microphoneDeviceId(sourceId)).toThrow('selected microphone is invalid')
  })
  ...
  it('lists Chromium microphone ids, labels, and a default device', async () => {
    ...
    await expect(listBrowserMicrophones()).resolves.toEqual([
      { id: 'microphone:chromium:default', kind: 'microphone', label: 'Microphone 1', isDefault: true },
      { id: 'microphone:chromium:usb', kind: 'microphone', label: 'USB microphone', isDefault: false },
    ])
  })
