...
type CameraSegmentApi = {
  beginCameraSegment(payload: { sessionId: string; sourceId: string; format: CameraFormat; startNs: number }): Promise<{ jobId: string }>
...
export class BrowserCameraRecorder {
  ...
  private async startSegment(sessionId: string, startNs: number) {
    ...
    const opened = await api().beginCameraSegment({ sessionId, sourceId: this.sourceId, format: this.format, startNs })
