...
describe('HudPreferences', () => {
  beforeEach(() => { setActivePinia(createPinia()); window.matchMedia ??= () => ({ matches: false, addEventListener: () => undefined }) as MediaQueryList })
  it('relays recording preferences and closes', async () => { const wrapper = mount(HudPreferences, { props: { recordHighQuality: true, countdownSeconds: 3 }, global: { stubs: { Switch, Select } } }); await wrapper.get('.quality').trigger('click'); await wrapper.get('.countdown').trigger('click'); const returnToHud = wrapper.findAll('button').find((button) => button.text().includes('Return to HUD')); await returnToHud?.trigger('click'); expect(wrapper.emitted('update:recordHighQuality')).toContainEqual([false]); expect(wrapper.emitted('update:countdownSeconds')).toContainEqual([10]); expect(wrapper.emitted('close')).toHaveLength(1) })
