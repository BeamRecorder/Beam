...
describe('TopbarHUD', () => {
  it('renders the default brand identity', () => { const wrapper = mount(TopbarHUD); expect(wrapper.get('img').attributes('alt')).toBe('Beam'); expect(wrapper.text()).toContain('Beam'); expect(wrapper.find('.rec-badge').exists()).toBe(false) })
  ...
  it('does not start a native drag when clicking an icon button', async () => {
    ...
    await wrapper.get('[aria-label="Preferences"] svg').trigger('pointerdown', { button: 0, pointerId: 1, clientX: 10, clientY: 10 })
