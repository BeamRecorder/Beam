...
const ShortcutInput = {
  ...
  template: '<div class="shortcut-stub"><span v-if="error" class="shortcut-error">{{ error }}</span><button class="change-shortcut" @click="$emit(\'update:modelValue\', modelValue === \'Alt+Shift+R\' ? \'Alt+Shift+R\' : \'Ctrl+Shift+X\')">change</button><button class="reset-shortcut" @click="$emit(\'reset\')">reset</button></div>',
}
...
describe('ShortcutPreferences', () => {
  ...
  it('persists a changed shortcut and displays a persistence error', async () => {
    ...
    expect(capture.updatePreferences).toHaveBeenCalledWith(expect.objectContaining({ shortcuts: expect.objectContaining({ 'hud.toggleMic': expect.objectContaining({ keys: 'Ctrl+Shift+X' }) }) }))
