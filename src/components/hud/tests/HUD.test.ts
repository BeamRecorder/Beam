...
describe('HUD', () => {
  beforeEach(() => { vi.useFakeTimers(); Object.values(capture).forEach((mock) => mock.mockReset()); Object.values(browserCameraMock).forEach((mock) => mock.mockReset()); Object.values(browserMicrophoneMock).forEach((mock) => mock.mockReset()); browserCameraMock.listBrowserCameras.mockResolvedValue([{ id: 'camera:chromium:device-1', kind: 'camera', label: 'Cam', isDefault: true }]); browserMicrophoneMock.listBrowserMicrophones.mockResolvedValue([{ id: 'microphone:chromium:device-1', kind: 'microphone', label: 'Mic', isDefault: true }]); browserCameraMock.request.mockResolvedValue({ onFatal: vi.fn(), start: vi.fn(), stop: vi.fn(), fail: vi.fn() }); browserMicrophoneMock.request.mockResolvedValue({ onFatal: vi.fn(), start: vi.fn(), stop: vi.fn(), fail: vi.fn() }); capture.discover.mockResolvedValue(catalog); capture.getSources.mockResolvedValue([{ id: 'screen:1', name: 'Display', thumbnail: '', appIcon: null }]) })
  ...
  it('discovers defaults and starts a screen recording with selected sources', async () => {
    ...
    expect(capture.startRecording).toHaveBeenCalledWith(expect.objectContaining({ screenKind: 'display', screenId: 'display:1', microphoneId: null, cameraId: 'camera:chromium:device-1', systemAudio: true, targetFps: 60 })); expect(browserCameraMock.request).toHaveBeenCalledWith('camera:chromium:device-1'); expect(browserMicrophoneMock.request).toHaveBeenCalledWith('microphone:chromium:device-1'); expect(wrapper.emitted('start-recording')).toEqual([[{ state: 'recording', sessionId: '019f84dd-4d9d-7f61-ac30-5da50169ecbc' }]])
  })
