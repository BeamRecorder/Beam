...
const settle = async () => { await flushPromises(); await vi.advanceTimersByTimeAsync(300); await flushPromises() }
...
describe('ProjectPicker', () => {
  ...
  it('loads projects, uses current selection and opens it', async () => {
    ...
    expect(wrapper.text()).toContain('First'); expect(wrapper.text()).toContain('Unknown date'); const open = wrapper.findAll('button').find((button) => button.text().includes('Open project')); await open?.trigger('click'); expect(wrapper.emitted('open-project')).toEqual([[projects[1]]])
  })
  ...
  it('renames a project, rejects unchanged names, and handles rename failure', async () => {
    ...
    const findRename = () => wrapper.findAll('button').find((button) => button.text() === 'Rename'); await findRename()?.trigger('click'); await vi.advanceTimersByTimeAsync(300); const input = () => wrapper.findAll('.input-element')[0]; await input().setValue(' Renamed '); await input().trigger('keydown', { key: 'Enter' }); await settle(); expect(capture.renameProject).toHaveBeenCalledWith('one', 'Renamed')
    await findRename()?.trigger('click'); await vi.advanceTimersByTimeAsync(300); await input().trigger('keydown', { key: 'Enter' }); expect(capture.renameProject).toHaveBeenCalledTimes(1)
    capture.renameProject.mockRejectedValueOnce(new Error('rename blocked')); await findRename()?.trigger('click'); await vi.advanceTimersByTimeAsync(300); await input().setValue('Blocked'); await input().trigger('keydown', { key: 'Enter' }); await settle(); expect(wrapper.findAll('.input-element')).toHaveLength(1)
  })
  it('deletes a selected project and selects the next remaining project', async () => {
    ...
    const remove = wrapper.findAll('button').find((button) => button.text() === 'Delete'); await remove?.trigger('click'); await wrapper.vm.$nextTick(); await wrapper.get('button.btn-danger').trigger('click'); await settle(); expect(capture.deleteProject).toHaveBeenCalledWith('one'); expect(wrapper.text()).toContain('Second')
  })
  it('keeps video preview progress and resets it after mouse leave', async () => {
    ...
    vi.spyOn(HTMLMediaElement.prototype, 'play').mockResolvedValue(undefined); vi.spyOn(HTMLMediaElement.prototype, 'load').mockImplementation(() => undefined); await wrapper.get('.project-card').trigger('mouseenter'); await wrapper.vm.$nextTick();
    const video = wrapper.get('video').element as HTMLVideoElement; Object.defineProperty(video, 'duration', { value: 2 }); Object.defineProperty(video, 'currentTime', { value: 1, writable: true }); video.pause = vi.fn(); await wrapper.get('video').trigger('timeupdate'); expect(wrapper.find('.preview-progress-overlay').exists()).toBe(true); await wrapper.get('.project-card').trigger('mouseleave'); expect(video.pause).toHaveBeenCalledOnce(); expect(video.currentTime).toBe(.1)
  })
})
