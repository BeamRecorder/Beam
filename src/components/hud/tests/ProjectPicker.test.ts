...
describe('ProjectPicker', () => {
  ...
  it('deletes a selected project and selects the next remaining project', async () => {
    ...
    const remove = wrapper.findAll('button').find((button) => button.text() === 'Delete'); await remove?.trigger('click'); const deletes = wrapper.findAll('button').filter((button) => button.text() === 'Delete'); await deletes[deletes.length - 1]?.trigger('click'); await settle(); expect(capture.deleteProject).toHaveBeenCalledWith('one'); expect(wrapper.text()).toContain('Second')
  })
