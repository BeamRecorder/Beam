...
describe('useProjectThumbnailGenerator', () => {
  ...
  beforeEach(() => {
    ...
    media.UrlSource.mockImplementation(function UrlSourceMock(url) {
      return { url };
    });
    ...
    vi.spyOn(HTMLCanvasElement.prototype, 'toDataURL').mockReset().mockReturnValue('data:image/webp;base64,thumbnail');
  });
  ...
  it('generates, saves and caches a thumbnail at the middle of the video', async () => {
    ...
    expect(media.UrlSource).toHaveBeenCalledOnce();
  });

  it('returns null for empty, invalid and trackless sources', async () => {
    const { generateThumbnail } = useProjectThumbnailGenerator();
    await expect(generateThumbnail('empty-project', '')).resolves.toBeNull();
    expect(media.UrlSource).not.toHaveBeenCalled();

    media.UrlSource.mockImplementationOnce(() => {
  ...
  it('times out stalled media inspection and disposes the input', async () => {
    vi.useFakeTimers();
    input.getPrimaryVideoTrack.mockReturnValue(new Promise(() => undefined));
    const { generateThumbnail } = useProjectThumbnailGenerator();

    const result = generateThumbnail('stalled-project', 'video://stalled');
