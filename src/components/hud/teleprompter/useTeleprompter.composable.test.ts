...
const storedDocument = (): TeleprompterDocument => ({
  schemaVersion: 1, text: 'stored one\nstored two', mode: 'line-by-line', autoscroll: false,
  scrollSpeed: 30, fontSize: 40, lineHeight: 1.4, textAlign: 'center', theme: 'dark', updatedAtUtc: 'yesterday',
})
...
describe('useTeleprompter composable', () => {
  ...
  beforeEach(() => {
    ...
    wrapper = mount(defineComponent({ setup() { api = useTeleprompter(); return () => null } }))
  })
  ...
  it('loads a new or stored session and handles persisted errors', async () => {
    ...
    expect(captureMock.saveSessionTeleprompter).toHaveBeenCalledWith('project', 'session', expect.objectContaining({ schemaVersion: 1 }))
  ...
  it('updates and saves the document, handles shortcuts and reports save failures', async () => {
    ...
    api.updateDocument({ text: 'one\ntwo\nthree', mode: 'line-by-line', autoscroll: true })
  ...
  it('autoscrolls continuously and by line, scrolls the active line, pauses and resumes', async () => {
    ...
    api.updateDocument({ text: 'first\nsecond\nthird', mode: 'continuous', autoscroll: true, scrollSpeed: 100 })
    ...
    api.updateDocument({ mode: 'line-by-line', autoscroll: true, scrollSpeed: 200 })
