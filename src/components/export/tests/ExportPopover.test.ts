...
const Button = {
  ...
  emits: ["click"],
  template:
    '<button v-bind="$attrs" class="button-stub" @click="$emit(\'click\')"><slot /></button>',
};
const ButtonGroup = {
...
describe("ExportPopover", () => {
  ...
  it("selects format and quality, then reports an unsupported codec", async () => {
    ...
    await wrapper
      .findAll(".export-popover .button-stub")
      .at(-1)
      ?.trigger("click");
    expect(supportedVideoCodec).toHaveBeenCalledWith(
      expect.objectContaining({ format: "mp4", preset: "high" }),
    );
  ...
  it("starts an export, displays its result, and opens the generated file", async () => {
    ...
    expect(mockJob.start).toHaveBeenCalledWith(
      expect.objectContaining({ format: "webm", preset: "medium" }),
    );
    ...
    expect(
      (window.capture as unknown as { openFile: ReturnType<typeof vi.fn> })
        .openFile,
    ).toHaveBeenCalledWith("C:\\Exports\\demo.webm");
  });
  ...
  it("renders export progress, percentage, elapsed time and cancellation", async () => {
    const wrapper = mountExport();
    const progress = mockJob.state?.progress as Ref<Record<
      string,
      unknown
    > | null>;
