...
function drawCaption(ctx: CanvasRenderingContext2D, clip: CaptionClip, timeMs: number, window: { x: number; y: number; width: number; height: number }, referenceWidth: number) {
  ...
  const box = { x: window.x + transform.x * window.width, y: window.y + transform.y * window.height, width: transform.width * window.width, height: transform.height * window.height };
...
function drawVisualClip(ctx: CanvasRenderingContext2D, clip: VisualClip, source: CanvasImageSource, canvas: { width: number; height: number }, window?: { x: number; y: number; width: number; height: number }) {
  ...
  drawDecoratedMedia(ctx, {
    source,
    sourceRect: clip.crop && sourceWidth > 0 && sourceHeight > 0 ? { x: clip.crop.x * sourceWidth, y: clip.crop.y * sourceHeight, width: clip.crop.width * sourceWidth, height: clip.crop.height * sourceHeight } : undefined,
    rect: { x: target.x + transform.x * target.width, y: target.y + transform.y * target.height, width: transform.width * target.width, height: transform.height * target.height },
...
export function drawCompositionLayers(ctx: CanvasRenderingContext2D, snapshot: CompositionSnapshot, time: number, visuals: CompositionVisuals = new Map(), positionedMedia?: { x: number; y: number; width: number; height: number }, mainVideoWidth = snapshot.video.width) {
  ...
  for (const clip of clips) {
    if (clip.kind === "caption") { drawCaption(ctx, clip, timeMs, positionedMedia ?? { x: 0, y: 0, width: snapshot.canvas.width, height: snapshot.canvas.height }, mainVideoWidth); continue; }
...
export function renderCompositionFrame(ctx: CanvasRenderingContext2D, video: RenderableVideo | null, snapshot: CompositionSnapshot, time: number, background?: CanvasImageSource | null, cursorImages?: ReadonlyMap<string, HTMLImageElement>, visuals?: CompositionVisuals) {
  const { width, height } = snapshot.canvas;
  ...
  const source = snapshot.canvas.showBackground ? { x: cropX, y: cropY, width: cropWidth, height: cropHeight } : coverSourceRect(cropWidth, cropHeight, width, height);
  ...
  const positionedMedia = { x: media.x + screen.transform.x * media.width, y: media.y + screen.transform.y * media.height, width: media.width * screen.transform.width, height: media.height * screen.transform.height };
  ...
  for (const clip of visualStack) {
    if (clip.kind === "screen") { drawScreen(); continue; }
    const sourceVisual = visuals?.get(clip.assetId);
    if (!sourceVisual) continue;
    if (clip.kind === "webcam") drawWebcamOverlay(ctx, sourceVisual, width, height, scale, webcamSettingsForAppearance(clip.appearance, clip.isMirrored), clip.transform, clip.crop, clip.appearance, clip.name);
