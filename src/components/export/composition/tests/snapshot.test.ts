...
describe('createCompositionSnapshot', () => {
  ...
  it('clamps invalid render metadata and duration without inventing cursor data', () => {
    const snapshot = createCompositionSnapshot({ ...base(), duration: -1, width: 0, height: -8, fps: 0, composition: { ...composition(), assets: [{ ...composition().assets[0], width: null, height: null }] } })
  ...
  it('copies reactive editor data and composition without retaining Vue proxies', () => {
    ...
    const snapshot = createCompositionSnapshot({ ...base(), editorData: editorData as never, composition: reactiveComposition })
