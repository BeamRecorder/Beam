...
const screenSourceFor = (request: ExportRequest) => {
  const screen = request.snapshot.composition.clips.find(
...
export const supportedVideoCodec = (request: ExportRequest) =>
  getFirstEncodableVideoCodec([...videoCodecs[request.format]], {
...
export async function supportedAudioCodec(request: ExportRequest) {
  ...
  return getFirstEncodableAudioCodec([...audioCodecs[request.format]], {
    sampleRate: 48_000,
    numberOfChannels: 2,
    bitrate: 128_000,
  });
...
export async function renderMixedAudio(request: ExportRequest): Promise<AudioBuffer | null> {
  ...
  const clips = request.snapshot.composition.clips.filter(
    (clip): clip is AudioClip => isAudioClip(clip) && clip.enabled,
  );
  ...
  await Promise.all(
    clips.map(async (clip) => {
      ...
      source.start(
        clip.timelineStartMs / 1_000,
        offset,
        Math.min(Math.max(0, buffer.duration - offset), clip.sourceDurationMs / 1_000),
      );
    }),
  );
...
const waitFor = (target: HTMLMediaElement, event: 'loadedmetadata' | 'seeked') =>
...
async function loadVisuals(request: ExportRequest) {
  ...
  await Promise.all(
    request.snapshot.composition.assets
      .filter((asset) => asset.kind !== 'audio' && asset.src)
      .map(async (asset) => {
        if (asset.kind === 'image') {
...
async function visualsAtTime(
  ...
  loaded: Awaited<ReturnType<typeof loadVisuals>>,
  time: number,
  ...
  for (const clip of activeClipsAt(request.snapshot.composition, time * 1_000)) {
    ...
    if (Math.abs(video.currentTime - local) > 0.001) {
...
export async function exportWithMediabunny(
  ...
  fallbackVideo.src = screenAsset.src;
  ...
  const source = new CanvasSource(canvas, {
    codec,
    bitrate: bitrateFor(request.preset, canvas.width, canvas.height, fps),
  });
  ...
  try {
    ...
    screenFrames = await VideoFrameProvider.create(
      screenAsset.src,
      times.map((time) => Math.max(0, (sourceTimeAt(screen, time * 1_000) ?? screen.sourceInMs) / 1_000)),
    );
    if (!screenFrames) {
      fallbackVideo.load();
    ...
    const videoAssets = request.snapshot.composition.assets.filter(
      (asset) => asset.kind === 'video' && asset.id !== screenAsset.id && asset.src,
    );
    await Promise.all(
      videoAssets.map(async (asset) => {
        const timestamps = times.map((time) => {
          const clip = activeClipsAt(request.snapshot.composition, time * 1_000).find(
            (item) => isVisualClip(item) && item.assetId === asset.id,
          );
    ...
    for (let frame = 0; frame < total; frame += 1) {
      ...
      if (!screenFrames && screenMs !== null && Math.abs(fallbackVideo.currentTime - screenMs / 1_000) > 0.001) {
        fallbackVideo.currentTime = screenMs / 1_000;
      ...
      const decodedFrames = await Promise.all(
        [...clipFrames].map(async ([assetId, provider]) => [assetId, await provider.frameAt(frame)] as const),
      );
      ...
      try {
        ...
        screenFrame?.close();
        decodedFrames.forEach(([, value]) => value?.close());
      }
    ...
    clipFrames.forEach((provider) => provider.dispose());
