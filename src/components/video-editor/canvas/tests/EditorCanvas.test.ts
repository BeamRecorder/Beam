...
const webcam = (): VisualClip => ({
  id: "webcam", kind: "webcam", name: "Camera", assetId: "camera-asset", timelineStartMs: 0,
  timelineDurationMs: 2_000, sourceInMs: 0, sourceDurationMs: 2_000, playbackRate: 1, enabled: true,
  order: 1, transform: { x: 0.1, y: 0.1, width: 0.3, height: 0.3 },
});

const image = (): VisualClip => ({
  id: "image", kind: "image", name: "Image", assetId: "image-asset", timelineStartMs: 0,
  timelineDurationMs: 2_000, sourceInMs: 0, sourceDurationMs: 2_000, playbackRate: 1, enabled: true,
  order: 2, transform: { x: 0.5, y: 0.4, width: 0.2, height: 0.2 },
});
...
describe("EditorCanvas", () => {
  it("renders the fallback stack, loading state, preview frame, and canvas pointer handlers", async () => {
    const mounted = mountEditor({ selectedZoom: { id: "zoom", mode: "manual", start: 0, end: 1, focus: { x: 0.5, y: 0.5 }, strength: 1 } });
    await flushPromises();
    runFrame();
