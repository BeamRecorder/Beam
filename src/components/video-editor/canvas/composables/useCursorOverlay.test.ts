...
const createContext = () => ({
  save: vi.fn(), restore: vi.fn(), measureText: vi.fn(() => ({ width: 70 })), roundRect: vi.fn(), fill: vi.fn(), fillText: vi.fn(),
  beginPath: vi.fn(), arc: vi.fn(), stroke: vi.fn(), drawImage: vi.fn(), translate: vi.fn(), scale: vi.fn(),
  font: '', textAlign: '', fillStyle: '', strokeStyle: '', lineWidth: 0, shadowColor: '', shadowBlur: 0, shadowOffsetX: 0, shadowOffsetY: 0,
...
describe('useCursorOverlay', () => {
  ...
  it('draws cursor, shadow, click ripple and custom-cursor warning', async () => {
    ...
    overlay.updateAndDrawRipplesAndCursor(ctx, { dx: 0, dy: 0, dw: 800, dh: 450, focusX: 0, focusY: 0, scale: 1 }, 1920, 1080, 800, drawContent)
  ...
  it('warns when cursor data is unavailable and removes expired ripples', () => {
    ...
    const overlay = useCursorOverlay(options)
    const ctx = createContext()
    overlay.updateAndDrawRipplesAndCursor(ctx, { dx: 0, dy: 0, dw: 800, dh: 450, focusX: 0, focusY: 0, scale: 1 }, 1920, 1080, 800, vi.fn())
