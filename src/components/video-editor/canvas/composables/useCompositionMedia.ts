...
export function useCompositionMedia(options: UseCompositionMediaOptions) {
  ...
  const reconcile = () => {
    ...
    for (const asset of assets.values()) {
      ...
      if (asset.kind === "image") {
        if (!images.has(asset.id)) { const image = new Image(); image.src = asset.src; images.set(asset.id, image); }
        continue;
      }
      ...
      const ready = () => {
        ...
        if (pending !== undefined) { pendingSeeks.delete(media); seek(media, pending); }
  ...
  const syncVideos = () => {
    ...
    const active = activeClipsAt(composition, timeMs).filter((clip): clip is VisualClip => isVisualClip(clip) && clip.kind !== "screen");
    ...
    for (const clip of composition.clips) {
      ...
      if (!media) continue;
      if (!activeIds.has(clip.id)) { media.pause(); continue; }
      const sourceMs = sourceTimeAt(clip, timeMs);
      if (sourceMs === null) { media.pause(); continue; }
  ...
  watch(() => [options.currentTime(), options.isPlaying(), options.composition()] as const, syncVideos, { flush: "post", deep: true });
  ...
  const drawCaption = (
    ...
    const transform = clip.id === selected?.id && options.transformDraft() ? options.transformDraft()! : getCaptionTransform(clip);
