...
const screenClip = (enabled = true): VisualClip => ({
  id: "screen", kind: "screen", name: "Screen", assetId: "screen-asset", timelineStartMs: 0,
  timelineDurationMs: 3_000, sourceInMs: 0, sourceDurationMs: 3_000, playbackRate: 1, enabled,
  order: 0, transform: { x: 0, y: 0, width: 1, height: 1 }, crop: { x: .1, y: .1, width: .8, height: .8 },
});
const composition = (enabled = true): ClipComposition => ({
  schemaVersion: 1,
  assets: [{ id: "screen-asset", kind: "video", name: "Screen", fileName: "screen.mp4", durationMs: 3_000, width: 1_280, height: 720, src: "screen.mp4", origin: "session" }],
  clips: [screenClip(enabled)],
});
const manualZoom: ZoomElement = { id: "manual", sessionId: "session", startMs: 0, endMs: 3_000, focus: { cx: .8, cy: .2 }, depth: 2, mode: "manual" };
const autoZoom: ZoomElement = { id: "auto", sessionId: "session", startMs: 0, endMs: 3_000, focus: { cx: .8, cy: .2 }, depth: 2, mode: "auto" };
const context = () => ({
  save: vi.fn(), restore: vi.fn(), beginPath: vi.fn(), roundRect: vi.fn(), clip: vi.fn(), fillRect: vi.fn(), fillText: vi.fn(),
  translate: vi.fn(), scale: vi.fn(), fillStyle: "", font: "", textAlign: "",
...
const mountComposable = () => {
  ...
  canvas.getBoundingClientRect = () => ({ left: 0, top: 0, width: 800, height: 450, right: 800, bottom: 450, x: 0, y: 0, toJSON: () => ({}) });
  ...
  const callbacks = {
    drawBackground: vi.fn(), onUpdateZoom: vi.fn(), onPreviewZoom: vi.fn(), onSelectScreenClip: vi.fn(),
    onSelectCanvas: vi.fn(), onDeselectTransformClip: vi.fn(), onDeselectZoom: vi.fn(), selectVisualAt: vi.fn(() => false),
  };
  const Harness = defineComponent({
    setup() {
      state = useCameraZoom({
        canvasRef: () => canvas, outputCanvas: () => output.value, zoomElements: () => zooms.value,
        selectedZoom: () => selected.value, currentTime: () => currentTime.value, isPlaying: () => false,
        editorData: () => ({
          cursor: {
            telemetry: [{ timeMs: 400, cx: .1, cy: .9 }, { timeMs: 900, cx: .9, cy: .1 }],
            events: [{ event: "move", sessionNs: 500_000_000, pixelX: 10, pixelY: 10, normalizedX: .2, normalizedY: .8, visible: true }],
          },
        } as any),
...
describe("useCameraZoom", () => {
  ...
  it("applies auto zoom, computes manual target focus, and updates pointer focus", async () => {
    ...
    const pointer = (type: string, x: number, y: number) => Object.assign(new MouseEvent(type, { clientX: x, clientY: y }), { pointerId: 4 }) as unknown as PointerEvent;
