...
export function coverSourceRect(sourceWidth: number, sourceHeight: number, destinationWidth: number, destinationHeight: number): CanvasRect {
...
export function containedMediaRect(sourceWidth: number, sourceHeight: number, destinationWidth: number, destinationHeight: number): CanvasRect {
...
export function framedMediaRect(sourceWidth: number, sourceHeight: number, destinationWidth: number, destinationHeight: number): CanvasRect {
  const frame = containedMediaRect(sourceWidth, sourceHeight, destinationWidth, destinationHeight)
  const scale = Math.min(.86, destinationWidth / frame.width, destinationHeight / frame.height)
  const width = frame.width * scale
  const height = frame.height * scale
  return { x: (destinationWidth - width) / 2, y: (destinationHeight - height) / 2, width, height }
}

export function coverPoint(cx: number, cy: number, sourceWidth: number, sourceHeight: number, destinationWidth: number, destinationHeight: number) {
