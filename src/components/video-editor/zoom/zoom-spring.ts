export interface CameraTransform { focusX: number; focusY: number; scale: number }
...
function stepCritical(current: number, target: number, velocity: number, deltaSeconds: number) {
  ...
  return { value: target + nextDisplacement, velocity: (velocity - omega * (velocity + omega * displacement) * deltaSeconds) * decay }
}

export function stepCameraSpring(current: CameraTransform, target: CameraTransform, velocity: CameraVelocity, deltaMs: number): CameraTransform {
  ...
  velocity.focusX = x.velocity; velocity.focusY = y.velocity; velocity.scale = scale.velocity
