...
describe("zoom playback", () => {
  it("returns null outside the envelope", () =>
    expect(zoomAtTime([zoom], 100)).toBeNull());
  it("uses the Depth scale at full strength", () =>
    expect(zoomAtTime([zoom], 4_000)?.scale).toBe(1.5));
  ...
  it("uses zero strength before an incoming region", () =>
    expect(regionStrength(zoom, 0)).toBe(0));
});
