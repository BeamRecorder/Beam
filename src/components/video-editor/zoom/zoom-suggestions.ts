...
export function normalizeCursorTelemetry(samples: CursorTelemetryPoint[], durationMs: number): CursorTelemetryPoint[] {
  return samples
    ...
    .map((sample) => ({ ...sample, timeMs: clamp(sample.timeMs, 0, durationMs), cx: clamp(sample.cx, 0, 1), cy: clamp(sample.cy, 0, 1) }))
...
interface ClickCluster { firstMs: number; lastMs: number; focus: ZoomFocus }
...
function clusterClicks(samples: CursorTelemetryPoint[]): ClickCluster[] {
  ...
  const flush = () => {
    const best = members.reduce((winner, sample) => interactionStrength(sample) > interactionStrength(winner) ? sample : winner)
    clusters.push({ firstMs: members[0].timeMs, lastMs: members.at(-1)?.timeMs ?? members[0].timeMs, focus: { cx: best.cx, cy: best.cy } })
  }
  for (const click of clicks.slice(1)) {
    ...
    else { flush(); members = [click] }
  }
  flush()
  return clusters
}

export function buildAutomaticZoomElements(params: { telemetry: CursorTelemetryPoint[]; sessionId: string; durationMs: number; reserved?: ZoomElement[] }): ZoomElement[] {
  ...
  return clusterClicks(normalizeCursorTelemetry(params.telemetry, params.durationMs)).flatMap((cluster) => {
    ...
    return [{ id: `auto:${params.sessionId}:${Math.round(cluster.firstMs)}`, sessionId: params.sessionId, startMs, endMs, focus: cluster.focus, depth: DEFAULT_ZOOM_DEPTH, mode: 'auto' as const }]
  })
}
