...
const sessionAsset = (
  ...
  return {
    ...
    name: track.kind === "screen" ? "Screen recording" : track.kind === "camera" ? "Webcam" : track.kind === "system-audio" ? "System audio" : "Microphone",
    fileName: null,
    durationMs,
    ...dimensions(track),
    src: segment.src ?? "",
...
export function synchronizeRecordingClips(
  ...
  for (const track of editorData.tracks) {
    ...
    for (const segment of track.assets) {
      ...
      const priority = track.kind === "camera" ? 20_000 : track.kind === "screen" ? 30_000 : track.kind === "system-audio" ? 40_000 : 50_000;
  ...
  if (!candidates.some((clip) => clip.kind === "screen") && !clips.some((clip) => clip.kind === "screen") && editorData.videoSrc) {
  ...
  for (const [key, group] of groups) {
    ...
    group.forEach((clip) => { clip.groupId = groupId; });
  }
