...
const segment = (
  overrides: Partial<SessionTrackAsset> = {},
): SessionTrackAsset => ({
  path: "segment.webm",
  startNs: 0,
  endNs: 2_000_000_000,
...
describe("synchronizeRecordingClips", () => {
  ...
  it("creates visual and audio clips, normalizes durations and groups synchronized tracks", () => {
    ...
    expect(audioClips.map((clip) => clip.role)).toEqual([
      "system",
      "microphone",
    ]);
    ...
    expect(
      result.assets.find(
        (asset) => asset.kind === "video" && asset.src === "/screen.webm",
      )?.width,
    ).toBe(1920);
    expect(
      result.assets.find((asset) => asset.src === "/mic.opus")?.width,
    ).toBeNull();
  });
  ...
  it("skips incomplete assets and creates a primary screen fallback when needed", () => {
    ...
    const result = synchronizeRecordingClips(
      ...
      editorData(
        [
          ...
          track(
            "camera",
            [segment({ path: "camera.webm", src: "/camera.webm" })],
            { placement: { x: "bad" } },
          ),
        ],
        "/primary.mp4",
    ...
    expect(result.assets.some((asset) => asset.src === "/primary.mp4")).toBe(
      true,
    );
  });
