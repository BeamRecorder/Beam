...
import { computeWebcamLayout, drawWebcamOverlay, getWebcamZoomFactor, webcamSettingsForAppearance } from '../webcam/webcam-zoom';

const context = () =>
  ({
    save: vi.fn(), restore: vi.fn(), beginPath: vi.fn(), roundRect: vi.fn(), clip: vi.fn(), fill: vi.fn(), stroke: vi.fn(),
    drawImage: vi.fn(), translate: vi.fn(), scale: vi.fn(), fillRect: vi.fn(), strokeRect: vi.fn(), fillText: vi.fn(),
    arc: vi.fn(), moveTo: vi.fn(), lineTo: vi.fn(), closePath: vi.fn(), createLinearGradient: vi.fn(() => ({ addColorStop: vi.fn() })),
    fillStyle: '', strokeStyle: '', lineWidth: 0, shadowColor: '', shadowBlur: 0, shadowOffsetX: 0, shadowOffsetY: 0,
    font: '', textAlign: '', textBaseline: '', lineCap: 'round', lineJoin: 'round',
...
describe('webcam zoom layout', () => {
  ...
  it('uses frame dimensions for crop and preserves both mirror axes', () => {
    ...
    drawWebcamOverlay(
      ...
      800,
      1,
      webcamSettingsForAppearance({ shadowSize: 'none', cornerRadius: 'none' }, true, true),
