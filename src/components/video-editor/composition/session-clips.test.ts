...
const segment = (overrides: Partial<SessionTrackAsset> = {}): SessionTrackAsset => ({
  path: 'segment.webm', startNs: 0, endNs: 2_000_000_000, complete: true, src: '/segment.webm', exists: true, ...overrides,
})
...
const track = (
  ...
  trackId: kind, kind, sourceId: kind, format, segments: assets, assets, metrics: {}, status, terminationReason: null,
})

const editorData = (tracks: SessionTrackData[], videoSrc: string | null = null): ProjectEditorData => ({
  sessionId: 'session-1', videoSrc,
  manifest: {
    schemaVersion: 1, projectId: 'project', sessionId: 'session-1', createdAtUtc: '', sessionStartMonotonicNs: 0,
    durationNs: 2_000_000_000, platform: {}, selectedSources: {}, tracks, permissions: {}, warnings: [], completed: true,
  },
  tracks,
...
describe('synchronizeRecordingClips', () => {
  ...
  it('creates visual and audio clips, normalizes durations and groups synchronized tracks', () => {
    ...
    const system = segment({ path: 'system.opus', startNs: 500_000_000, endNs: null, src: '/system.opus' })
    const microphone = segment({ path: 'mic.opus', endNs: 1_000_000, src: '/mic.opus' })
    const result = synchronizeRecordingClips(emptyComposition(), editorData([
      track('screen', [screen], { width: 1920, height: 1080 }),
      track('camera', [camera], { width: 640, height: 480, placement: { x: 0.1, y: 0.2, width: 0.3, height: 0.4 } }),
    ...
    expect(cameraClip).toMatchObject({ transform: { x: 0.1, y: 0.2, width: 0.3, height: 0.4 } })
  ...
  it('skips incomplete assets and creates a primary screen fallback when needed', () => {
    ...
    expect(result.clips.find((clip) => clip.kind === 'screen')).toMatchObject({ id: 'screen', timelineDurationMs: 2000, assetId: 'session:session-1:screen:primary' })
    expect(result.clips.find((clip) => clip.kind === 'webcam')).toMatchObject({ transform: { x: 0.72, y: 0.72, width: 0.24, height: 0.24 } })
  ...
  it('preserves existing clips and does not duplicate recording sources', () => {
    const composition: ClipComposition = {
      ...emptyComposition(),
      assets: [{ id: 'old', kind: 'image', name: 'Old', fileName: null, durationMs: 1000, width: 10, height: 10, src: '/old.png', origin: 'project' }],
      clips: [{ id: 'screen', kind: 'screen', name: 'Trimmed screen', assetId: 'old', timelineStartMs: 200, timelineDurationMs: 400, sourceInMs: 100, sourceDurationMs: 400, playbackRate: 1, enabled: true, order: 1, transform: { x: 0, y: 0, width: 1, height: 1 } }],
    }
