...
export const frameContentRect = (rect: MediaRect, frame: ClipFrame, windows: WindowsFrameOptions = {}): MediaRect => {
  ...
  if (frame === "windows-95") {
    ...
    return { x: rect.x + left, y: rect.y + top, width: Math.max(1, rect.width - left - right), height: Math.max(1, rect.height - top - bottom) };
  }
  return rect;
};

export const frameRadius = (frame: ClipFrame, fallback: number, rect: MediaRect) =>
  Math.min(frame === "safari" ? Math.max(5, rect.width * 18 / SAFARI_REFERENCE.width) : frame === "windows-95" ? 0 : fallback, rect.width / 2, rect.height / 2);
...
function drawSafariToolbar(ctx: CanvasRenderingContext2D, rect: MediaRect, title: string, paintBackground: boolean, chromeScale = 1) {
  ...
  if (paintBackground) {
    ctx.save();
    ctx.beginPath(); ctx.roundRect(rect.x, rect.y, rect.width, rect.height, radius); ctx.clip();
    ctx.fillStyle = "#fafafa"; ctx.fillRect(rect.x, rect.y, rect.width, rect.height);
    const background = ctx.createLinearGradient(rect.x, rect.y, rect.x, rect.y + header);
    background.addColorStop(0, "#fdfdfd"); background.addColorStop(1, "#f8f8f8");
    ctx.fillStyle = background; ctx.fillRect(rect.x, rect.y, rect.width, header);
    ctx.restore();
  }

  ctx.save();
  ctx.lineWidth = line; ctx.strokeStyle = "rgba(75, 75, 75, .93)"; ctx.lineCap = "round"; ctx.lineJoin = "round";
  const circle = (cx: number, color: string) => { ctx.beginPath(); ctx.arc(x(cx), y(34), 7.5 * Math.min(scale.x, scale.y), 0, Math.PI * 2); ctx.fillStyle = color; ctx.fill(); ctx.strokeStyle = "rgba(0, 0, 0, .08)"; ctx.lineWidth = .75 * Math.min(scale.x, scale.y); ctx.stroke(); ctx.strokeStyle = "rgba(75, 75, 75, .93)"; ctx.lineWidth = line; };
  circle(31, "#ff5f57"); circle(57, "#febc2e"); circle(83, "#28c840");

  // left-controls: sidebar, disclosure, backward and forward placeholders
  ctx.strokeRect(x(124), y(24), 24 * scale.x, 20 * scale.y); safariPath(ctx, [[x(132), y(24)], [x(132), y(44)]]);
  safariPath(ctx, [[x(164), y(31)], [x(168), y(35)], [x(172), y(31)]]);
  safariPath(ctx, [[x(217), y(26)], [x(208), y(34)], [x(217), y(42)]]);
  safariPath(ctx, [[x(251), y(26)], [x(260), y(34)], [x(251), y(42)]]);

  // privacy-control placeholder
  ctx.fillStyle = "#505050"; ctx.beginPath(); ctx.moveTo(x(509), y(23)); ctx.lineTo(x(519), y(27)); ctx.lineTo(x(517), y(39)); ctx.lineTo(x(509), y(45)); ctx.lineTo(x(501), y(39)); ctx.lineTo(x(499), y(27)); ctx.closePath(); ctx.fill();

  // address-bar and centered address-content
  ctx.fillStyle = "#f7f7f7"; ctx.beginPath(); ctx.roundRect(x(538), y(17), 726 * scale.x, 34 * scale.y, 9 * Math.min(scale.x, scale.y)); ctx.fill();
  ctx.strokeStyle = "#d2d2d2"; ctx.lineWidth = line; ctx.stroke();
  ctx.strokeStyle = "rgba(255, 255, 255, .8)"; safariPath(ctx, [[x(548), y(18.5)], [x(1254), y(18.5)]]);
  ctx.strokeStyle = "#777777"; ctx.lineWidth = Math.max(.6, line * .9); ctx.beginPath(); ctx.roundRect(x(858), y(31), 10 * scale.x, 10 * scale.y, 1.5 * Math.min(scale.x, scale.y)); ctx.stroke(); safariPath(ctx, [[x(860.5), y(31)], [x(860.5), y(27.5)], [x(865), y(25.5)], [x(868), y(27.5)], [x(868), y(31)]]);
  const addressTitle = title || "website.com";
  ctx.fillStyle = "#565656"; ctx.font = `${Math.max(8, 16 * Math.min(scale.x, scale.y))}px -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif`; ctx.textAlign = "left"; ctx.textBaseline = "middle"; ctx.fillText(addressTitle, x(876), y(34));
  ctx.strokeStyle = "rgba(75, 75, 75, .93)"; ctx.lineWidth = line; ctx.beginPath(); ctx.arc(x(1244), y(34), 8 * Math.min(scale.x, scale.y), -.8, Math.PI * 1.45); ctx.stroke(); safariPath(ctx, [[x(1250), y(27)], [x(1251), y(33)], [x(1245), y(31)]]);

  // right-controls placeholders: share, add tab, tab overview
  ctx.strokeRect(x(1662), y(28), 16 * scale.x, 15 * scale.y); safariPath(ctx, [[x(1670), y(34)], [x(1670), y(21)], [x(1666), y(25)]]); safariPath(ctx, [[x(1670), y(21)], [x(1674), y(25)]]);
  safariPath(ctx, [[x(1707), y(34)], [x(1725), y(34)]]); safariPath(ctx, [[x(1716), y(25)], [x(1716), y(43)]]);
  ctx.strokeRect(x(1755), y(25), 16 * scale.x, 15 * scale.y); ctx.strokeRect(x(1759), y(29), 16 * scale.x, 15 * scale.y);
  ctx.restore();

  ctx.save(); ctx.strokeStyle = "rgba(213, 213, 213, .75)"; ctx.lineWidth = line; ctx.beginPath(); ctx.moveTo(rect.x + line / 2, rect.y + header - line / 2); ctx.lineTo(rect.x + rect.width - line / 2, rect.y + header - line / 2); ctx.stroke(); ctx.restore();
}

function drawBevelEdges(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, raised: boolean) {
  ctx.fillStyle = raised ? "#ffffff" : "#808080"; ctx.fillRect(x, y, width, 1); ctx.fillRect(x, y, 1, height);
  ctx.fillStyle = raised ? "#808080" : "#ffffff"; ctx.fillRect(x, y + height - 1, width, 1); ctx.fillRect(x + width - 1, y, 1, height);
  ctx.fillStyle = raised ? "#dfdfdf" : "#404040"; ctx.fillRect(x + 1, y + 1, Math.max(0, width - 2), 1); ctx.fillRect(x + 1, y + 1, 1, Math.max(0, height - 2));
}

function drawBevel(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, raised: boolean, color: string) {
...
function drawOuterWindowsBevel(ctx: CanvasRenderingContext2D, rect: MediaRect) {
  ctx.fillStyle = "#ffffff"; ctx.fillRect(rect.x, rect.y, rect.width, 1); ctx.fillRect(rect.x, rect.y, 1, rect.height);
  ctx.fillStyle = "#dfdfdf"; ctx.fillRect(rect.x + 1, rect.y + 1, Math.max(0, rect.width - 3), 1); ctx.fillRect(rect.x + 1, rect.y + 1, 1, Math.max(0, rect.height - 3));
  ctx.fillStyle = "#000000"; ctx.fillRect(rect.x, rect.y + rect.height - 1, rect.width, 1); ctx.fillRect(rect.x + rect.width - 1, rect.y, 1, rect.height);
  ctx.fillStyle = "#808080"; ctx.fillRect(rect.x + 1, rect.y + rect.height - 2, Math.max(0, rect.width - 3), 1); ctx.fillRect(rect.x + rect.width - 2, rect.y + 1, 1, Math.max(0, rect.height - 3));
}
...
function drawWindows95Frame(ctx: CanvasRenderingContext2D, rect: MediaRect, title: string, paintBackground: boolean, color: string, windows: WindowsFrameOptions) {
  ...
  if (windows.showScrollbars !== false) { const scrollbar = Math.max(12, Math.round(18 * Math.min(sx, sy))); const verticalX = rect.x + rect.width - scrollbar - Math.max(2, Math.round(4 * sx)); const horizontalY = rect.y + rect.height - scrollbar - Math.max(2, Math.round(4 * sy)); drawBevel(ctx, verticalX, content.y, scrollbar, scrollbar, true, color); drawBevel(ctx, verticalX, rect.y + rect.height - scrollbar * 2 - Math.max(2, Math.round(4 * sy)), scrollbar, scrollbar, true, color); ctx.fillStyle = color; ctx.fillRect(verticalX, content.y + scrollbar, scrollbar, Math.max(1, horizontalY - content.y - scrollbar)); drawBevel(ctx, verticalX + 2, content.y + scrollbar + 8, Math.max(1, scrollbar - 4), Math.max(12, Math.round(48 * sy)), true, color); drawBevel(ctx, content.x, horizontalY, scrollbar, scrollbar, true, color); drawBevel(ctx, verticalX - scrollbar, horizontalY, scrollbar, scrollbar, true, color); ctx.fillStyle = color; ctx.fillRect(content.x + scrollbar, horizontalY, Math.max(1, verticalX - content.x - scrollbar), scrollbar); drawBevel(ctx, content.x + scrollbar + 8, horizontalY + 2, Math.max(12, Math.round(72 * sx)), Math.max(1, scrollbar - 4), true, color); ctx.fillStyle = color; ctx.fillRect(verticalX, horizontalY, scrollbar, scrollbar); ctx.strokeStyle = "#808080"; ctx.lineWidth = 1; for (let i = 0; i < 3; i += 1) { const offset = 5 + i * 4; ctx.beginPath(); ctx.moveTo(verticalX + offset, horizontalY + scrollbar - 3); ctx.lineTo(verticalX + scrollbar - 3, horizontalY + offset); ctx.stroke(); } }
}
