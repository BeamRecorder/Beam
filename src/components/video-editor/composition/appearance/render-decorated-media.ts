...
export function applyClipShadow(
  ...
  ctx.shadowBlur = blur
  ctx.shadowOffsetX =
    style.shadowDirection === 'top-left' ? -width * 0.018 : style.shadowDirection === 'bottom-right' ? width * 0.018 : 0
  ctx.shadowOffsetY =
...
export function drawDecoratedMedia(ctx: CanvasRenderingContext2D, options: DecoratedMediaOptions) {
  ...
  if (scaleX !== 1 || scaleY !== 1) {
    ctx.translate(scaleX === -1 ? content.x * 2 + content.width : 0, scaleY === -1 ? content.y * 2 + content.height : 0)
