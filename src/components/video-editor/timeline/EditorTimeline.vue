...
const handleKeyDown = (event: KeyboardEvent) => {
  ...
  if (tag === "input" || tag === "textarea" || tag === "select" || active?.getAttribute("contenteditable") === "true") return;
...
<template>
  <div class="timeline-island-container">
    <TimelineTracks
      ...
      @split:clip="emit('split:clip', $event)"
...
<style scoped>
.timeline-island-container { width: 100%; background: var(--color-bg-element); border-radius: var(--radius-lg); border: 1px solid var(--color-border); box-shadow: var(--shadow-sm); overflow: hidden; display: flex; flex-direction: column; gap: 0; }
</style>
