...
const asset = (kind: MediaAsset['kind'], src = `/media/${kind}`): MediaAsset => ({
  id: `${kind}-asset`, kind, name: kind, fileName: null, durationMs: 10_000,
  width: kind === 'audio' ? null : 1280, height: kind === 'audio' ? null : 720,
  src, origin: 'project',
})

const clip = (overrides: Partial<Clip> = {}): Clip => ({
  id: 'clip-1', kind: 'video', name: 'A very long recording title', timelineStartMs: 1_000,
  timelineDurationMs: 2_000, sourceInMs: 0, sourceDurationMs: 2_000, playbackRate: 1.25,
  enabled: true, order: 0, assetId: 'video-asset',
  transform: { x: 0, y: 0, width: 1, height: 1 }, ...overrides,
} as Clip)

const baseProps = {
  clip: clip(), asset: asset('video', '/video.mp4'), duration: 10,
  visibleSeconds: [0, 1, 2, 3], selected: true,
}
...
describe('TimelineClip', () => {
  ...
  it('renders audio waveforms, image previews and unavailable waveform labels', async () => {
    const audio = mount(TimelineClip, {
      props: {
        ...baseProps,
        clip: clip({ kind: 'audio', assetId: 'audio-asset', name: 'Audio', playbackRate: 1, enabled: false }),
        asset: asset('audio'), waveformBars: [4, 10], selected: false,
      },
      global: { stubs: { Skeleton } },
    })
    ...
    const unavailable = mount(TimelineClip, {
      props: { ...baseProps, clip: clip({ kind: 'audio', assetId: 'audio-asset' }), asset: asset('audio'), waveformBars: [] },
      global: { stubs: { Skeleton } },
    })
    expect(unavailable.find('.waveform-unavailable').exists()).toBe(true)

    const image = mount(TimelineClip, {
      props: { ...baseProps, clip: clip({ kind: 'image', assetId: 'image-asset' }), asset: asset('image', '/poster.png') },
      global: { stubs: { Skeleton } },
    })
  ...
  it('marquees an overflowing label and stops it on leave and unmount', async () => {
    const wrapper = mount(TimelineClip, { props: baseProps, global: { stubs: { Skeleton } } })
