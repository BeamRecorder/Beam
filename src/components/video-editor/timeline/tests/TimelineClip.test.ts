...
beforeEach(() => {
  ...
  vi.spyOn(window, "requestAnimationFrame").mockImplementation((callback) => {
    const callCount = (window.requestAnimationFrame as ReturnType<typeof vi.fn>)
      .mock.calls.length;
...
describe("TimelineClip", () => {
  it("renders video frames, speed and trim state, then emits clip interactions", async () => {
    ...
    expect(wrapper.get(".timeline-clip").classes()).toEqual(
      expect.arrayContaining(["selected", "kind-video"]),
    );
    expect(wrapper.get(".timeline-clip").attributes("style")).toContain(
      "left: 10%",
    );
    expect(wrapper.get(".timeline-clip").attributes("style")).toContain(
      "width: 20%",
    ...
    expect(wrapper.find(".thumbnail-img").attributes("src")).toBe(
      "/thumb-0.png",
    );
    ...
    expect(wrapper.emitted("trim")?.[0]?.[0]).toEqual(
      expect.objectContaining({ edge: "end" }),
    );
  });
  ...
  it("renders audio waveforms, image previews and unavailable waveform labels", async () => {
    ...
    expect(audio.get(".timeline-clip").classes()).toEqual(
      expect.arrayContaining(["kind-audio", "disabled"]),
    );
