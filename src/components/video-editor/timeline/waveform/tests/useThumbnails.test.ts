...
vi.mock("./thumbnail.worker?worker&inline", () => ({
...
describe("useThumbnails", () => {
  ...
  const createObjectURL = vi.fn(
    (_blob: Blob | MediaSource): string => "blob:initial",
  );
  ...
  beforeEach(() => {
    ...
    createObjectURL.mockImplementation(
      () => `blob:${createObjectURL.mock.calls.length}`,
    );
  ...
  const ready = (
    generation: number,
    time: number,
    blob = new Blob([String(time)]),
  ) => {
    workerState.instances[0]?.onmessage?.({
      data: { type: "ready", generation, time, blob },
    } as MessageEvent);
  };
  ...
  it("queues visible frames, extracts missing thumbnails, updates state and evicts old cache entries", async () => {
    ...
    expect(worker.postMessage).toHaveBeenCalledWith({
      type: "request-frames",
      generation: 2,
      source: "safe://video.mp4",
      visibleTimes: [1, 2],
    });

    worker.onmessage?.({
      data: { type: "batch-started", generation: 2 },
    } as MessageEvent);
    ...
    worker.onmessage?.({
      data: { type: "batch-finished", generation: 2 },
    } as MessageEvent);
    ...
    worker.onmessage?.({
      data: { type: "error", generation: 2, message: "failed" },
    } as MessageEvent);
    expect(api.isExtracting.value).toBe(false);
    worker.onerror?.();
  });
