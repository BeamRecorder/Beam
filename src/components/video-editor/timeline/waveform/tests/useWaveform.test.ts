...
describe("useWaveform", () => {
  it("handles empty input and sends non-empty audio to a worker", () => {
    ...
    expect(worker.postMessage).toHaveBeenCalledWith(
      { type: "process", audioData: samples, targetPoints: 2 },
      [samples.buffer],
    );
    worker.onmessage?.({ data: { type: "progress", progress: 50, peaks: new Float32Array([-1, 0.5]) } } as MessageEvent);
  ...
  it("converts PCM, float WAV, unsupported formats, and audio buffers", async () => {
    ...
    worker.onmessage?.({ data: { type: "done", peaks: new Float32Array([-.5, 1]) } } as MessageEvent);
    ...
    const audioBuffer = { sampleRate: 2, length: 2, numberOfChannels: 2, getChannelData: (channel: number) => channels[channel] } as unknown as AudioBuffer;
