...
describe('thumbnail worker protocol', () => {
  ...
  it('accepts a complete frame request', () => {
    expect(isThumbnailWorkerRequest({
      type: 'request-frames', generation: 3, source: 'file:///recording.mp4', visibleTimes: [0, 1],
    })).toBe(true)
  })

  it('rejects incomplete and malformed worker messages', () => {
    expect(isThumbnailWorkerRequest({ type: 'request-frames', generation: 0, source: 'x', visibleTimes: ['1'] })).toBe(false)
