...
const barsFromAudioBuffer = (
  ...
  const endSample = Math.min(buffer.length, Math.ceil((sourceInMs + sourceDurationMs) / 1_000 * buffer.sampleRate));
  ...
  return Array.from({ length: Math.max(1, count) }, (_, index) => {
    ...
    for (let channel = 0; channel < buffer.numberOfChannels; channel += 1) {
      ...
      for (let sample = start; sample < Math.min(end, data.length); sample += 1) peak = Math.max(peak, Math.abs(data[sample] ?? 0));
    }
    return Math.max(2, Math.round(peak * 22));
  });
};

const barsFromMedia = async (src: string, sourceInMs: number, sourceDurationMs: number, count: number) => {
  ...
  try {
    ...
    const durationSeconds = Math.max(.001, sourceDurationMs / 1_000);
...
export function useCompositionAudioWaveforms(composition: () => ClipComposition, timelineDurationSeconds: () => number) {
  ...
  watch(sources, async (clips) => {
    ...
    await Promise.all(clips.map(async (clip) => {
      ...
      try {
        ...
        next[clip.id] = barsFromAudioBuffer(await context.decodeAudioData(await response.arrayBuffer()), clip.sourceInMs, clip.sourceDurationMs, count);
      } catch {
        try { next[clip.id] = await barsFromMedia(clip.src, clip.sourceInMs, clip.sourceDurationMs, count); }
        catch { next[clip.id] = []; }
      }
    }));
