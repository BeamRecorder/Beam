...
const buffer = (values = [0, 0.2, 0.4, 0.8, 0, 0.1, 0.3, 0.7]): AudioBuffer =>
  ({
    sampleRate: 10,
...
class FakeWorker {
  ...
  postMessage = vi.fn(() =>
    queueMicrotask(() =>
      this.onmessage?.({ data: { type: 'done', peaks: new Float32Array([0, 0.2, 0, 0.8]) } } as MessageEvent),
    ),
  )
}
...
beforeEach(() => {
  ...
  sinkBuffers.mockImplementation(async function* () {
    yield { buffer: buffer(), timestamp: 0, duration: 1 }
  })
})
...
describe('useCompositionAudioWaveforms', () => {
  ...
  it('returns empty bars for undecodable or missing sources and terminates workers', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      expect.stringContaining('Unable to calculate waveform'),
      expect.anything(),
    )
