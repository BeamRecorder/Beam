...
describe("sentencesFromWords", () => {
  ...
  it("splits long unpunctuated transcripts into editable phrases", () => {
    ...
    expect(
      sentencesFromWords(words).map((sentence) => sentence.words.length),
    ).toEqual([12, 1]);
  });
  it("prevents an imprecise word timestamp from overlapping the next caption", () => {
    ...
    expect(
      sentences.map((sentence) => [sentence.startMs, sentence.endMs]),
    ).toEqual([
      [0, 500],
      [500, 900],
    ]);
  });
  it("returns no phrases when Whisper returns no words", () =>
