...
class FakeWorker {
  ...
  addEventListener = vi.fn((_type: string, listener: (event: MessageEvent) => void) => { this.listener = listener })
  removeEventListener = vi.fn(() => undefined)
  terminate = vi.fn(() => { this.terminated = true })
  postMessage = vi.fn((message: { id: string }) => {
    this.messages.push(message)
    this.listener?.({ data: { id: 'other-request', type: 'progress', status: 'transcribing', message: 'ignored', progress: 0.1 } } as MessageEvent)
    if (workerState.mode === 'error') {
      this.listener?.({ data: { id: message.id, type: 'error', message: 'Whisper failed' } } as MessageEvent)
      return
    }
    this.listener?.({ data: { id: message.id, type: 'progress', status: 'transcribing', message: 'Working', progress: 0.5 } } as MessageEvent)
    this.listener?.({ data: {
      id: message.id,
      type: 'result',
      words: [
...
class FakeOfflineAudioContext {
  ...
  startRendering = vi.fn(async () => ({ getChannelData: () => new Float32Array([0.1, 0.2, 0.3]) }))
  constructor(channels: number, length: number, sampleRate: number) { this.args = [channels, length, sampleRate] }
}
...
describe('useWhisperTranscription composable', () => {
  ...
  it('normalizes audio, reports progress, returns sentences and terminates the worker', async () => {
    ...
    const wrapper = mount(defineComponent({ setup() { api = useWhisperTranscription(); return () => null } }))
    ...
    expect(workerState.instances[0]!.messages[0]).toEqual(expect.objectContaining({ model: 'Xenova/whisper-tiny', locale: expect.any(String), sampleRate: 16_000 }))
  ...
  it('rejects unreadable audio and worker transcription failures', async () => {
    ...
    await expect(api.transcribe('missing.wav', 'Xenova/whisper-tiny')).rejects.toThrow('Unable to read selected audio source.')

    workerState.mode = 'error'
    await expect(api.transcribe('audio.wav', 'Xenova/whisper-tiny')).rejects.toThrow('Whisper failed')
    expect(api.progress.value).toEqual({ status: 'error', message: 'Whisper failed' })
    wrapper.unmount()
  })
})
