...
export const sentencesFromWords = (words: CaptionWord[]): CaptionSentence[] => {
  ...
  const sentences = groups.map((items) => ({ id: crypto.randomUUID(), text: items.map((word) => word.text).join(' '), startMs: items[0].startMs, endMs: items.at(-1)!.endMs, words: items }))
...
const mono = async (src: string, maximumDurationMs?: number) => {
  ...
  const sampleRate = 16_000; const offline = new OfflineAudioContext(1, Math.ceil(sourceDuration * sampleRate), sampleRate)
