...
import { Box, Moon, Sparkles, Type } from "@lucide/vue";
  ...
  <div v-if="draft" class="caption-clip-panel">
    ...
    <section class="panel-group">
      ...
      <div class="field-grid-2">
        <div class="field-group"><label class="field-label">{{ t('shadowColor') }}</label><ColorPicker :model-value="captionStyle.shadowColor" @update:model-value="updateStyle('shadowColor', $event)" /></div>
        <div class="field-group"><label class="field-label">{{ t('direction') }}</label><Select :items="shadowDirectionOptions" :model-value="captionStyle.shadowDirection ?? 'bottom-right'" size="sm" @update:model-value="updateStyle('shadowDirection', $event as CaptionStyle['shadowDirection'])" /></div>
      </div>
    ...
    <Divider v-if="sentences.length" spacing="md" />
    ...
    <div class="danger-zone"><DeleteItem :label="t('deleteCaptionClip')" @click="emit('delete', draft.id)" /></div>
  </div>
</template>

<style scoped>
.caption-clip-panel { display: flex; flex: 1; min-height: 100%; flex-direction: column; gap: 4px; padding-bottom: 8px; }
.ai-badge-header { display: flex; align-items: center; gap: 6px; border: 1px solid rgba(99,102,241,.25); border-radius: var(--radius-sm); background: rgba(99,102,241,.12); color: var(--color-primary); padding: 7px 10px; font-size: 11px; font-weight: 700; margin-bottom: 6px; }
.ai-badge-icon { color: var(--color-primary); }.panel-group { display: flex; flex-direction: column; gap: 10px; padding-bottom: 2px; }
.group-title { display: flex; align-items: center; gap: 6px; margin: 0; color: var(--text-primary); font-size: 12px; font-weight: 700; }.group-description { margin: 3px 0 0; color: var(--text-muted); font-size: 11px; line-height: 1.35; }
.field-group { display: flex; flex-direction: column; gap: 5px; }.field-grid-2 { display: grid; grid-template-columns: minmax(0,1fr) minmax(0,1fr); gap: 10px; align-items: end; }
.field-label, .word-labels { color: var(--text-secondary); font-size: 10px; font-weight: 700; letter-spacing: .03em; text-transform: uppercase; }.field-description { margin: 0; color: var(--text-muted); font-size: 10px; line-height: 1.35; }
.timing-group { gap: 8px; }.word-labels, .word-row { display: grid; grid-template-columns: minmax(0,1fr) 62px 62px; gap: 6px; }.word-labels span:not(:first-child) { text-align: center; }
.sentence-box { display: flex; flex-direction: column; gap: 5px; }.sentence-text { margin: 0; color: var(--text-primary); font-size: 11px; font-weight: 700; }
