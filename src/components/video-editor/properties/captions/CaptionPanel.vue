...
<template>
  <div class="caption-sidebar-panel">
    ...
    <div class="config-card">
      <div class="field-group" v-if="sources.length"><label class="field-label">{{ t('audioSource') }}</label><Select :items="sourceSelectItems" :model-value="source" size="sm" @update:model-value="source = $event as TranscriptionSource" /></div>
      ...
      <Button v-if="!modelReady" variant="secondary" size="sm" :icon="Download" @click="downloadModel" block>Download Model</Button>
      ...
      <Button variant="primary" size="md" :icon="hasAiCaptions ? RefreshCw : Sparkles" :disabled="!modelReady || !selectedSource || progress.status === 'loading' || progress.status === 'running'" @click="runTranscription" block class="generate-btn">{{ progress.status === 'idle' ? (hasAiCaptions ? t('regenerateAICaptions') : t('generateCaptions')) : t('processing') }}</Button>
    </div>
    <Divider v-if="hasAiCaptions" spacing="sm" />
