...
describe('CaptionPanel', () => {
  ...
  it('loads a missing model, displays progress/errors and downloads it', async () => {
    ...
    await wrapper.get('button:not(.caption-select)').trigger('click')
  ...
  it('generates captions, replaces old AI captions, and exposes the edit action', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.find('.model-ready').exists()).toBe(true))
    expect(wrapper.find('.active-status-card').text()).toContain('1 subtitle track')
    await wrapper.get('.generate-btn').trigger('click')
    ...
    await wrapper.get('.active-status-card button').trigger('click')
