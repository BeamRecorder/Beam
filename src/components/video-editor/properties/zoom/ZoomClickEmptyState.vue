...
<template>
  <div class="zoom-empty-state empty-state">
...
/* Real Solid Zoom Indicator state */
.real-zoom-indicator {
  opacity: 0;
  background: var(--color-track-cursor, #f59e0b);
  box-shadow: var(--shadow-sm);
  animation: realZoomAnim 3.8s cubic-bezier(0.4, 0, 0.2, 1) infinite;
}
...
/* Animation Timings for exact sequence:
   0s - 0.8s: Cursor approaches Zooms track
   0.8s - 1.6s: Cursor HOVERS over track -> preview-ghost appears
   1.6s - 2.0s: Cursor CLICKS down -> preview-ghost turns into real-zoom-indicator with selection glow!
   2.0s - 3.2s: Zoom block stays selected & glowing
