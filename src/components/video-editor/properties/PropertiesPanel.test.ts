...
const CanvasPanel = { emits: ['update:selectedBackground'], template: '<button class="canvas-panel-stub" @click="$emit(\'update:selectedBackground\', { id: \'background\' })">Canvas</button>' }
...
const AudioClipPropertiesPanel = { props: ['clip'], template: '<div class="audio-clip-stub">{{ clip?.kind || "audio" }}</div>' }
...
const ClipPropertiesPanel = { props: ['selectedClip'], template: '<div class="clip-panel-stub">{{ selectedClip?.kind }}</div>' }
...
const global = { stubs: { CanvasPanel, AudioPanel, ZoomPanel, SettingsPanel, AudioClipPropertiesPanel, CaptionClipPanel, CaptionPanel, CursorPanel, ClipPropertiesPanel } }
...
describe('PropertiesPanel', () => {
  it('selects the correct child panel for every editor tab', async () => {
    ...
    const cases = [
      ['canvas', '.canvas-panel-stub'], ['audio', '.audio-panel-stub'], ['zoom', '.zoom-panel-stub'],
      ['settings', '.settings-panel-stub'], ['cursor', '.cursor-panel-stub'], ['caption', '.caption-panel-stub'],
    ] as const
  ...
  it('selects audio, caption and regular clip property editors', async () => {
    const wrapper = mount(PropertiesPanel, { props: baseProps, global })
    await wrapper.setProps({ activeTab: 'clip', selectedClip: { id: 'audio', kind: 'audio', timelineStartMs: 0, timelineDurationMs: 100 } })
    ...
    await wrapper.setProps({ selectedCaptionClip: null, selectedClip: { id: 'screen', kind: 'screen', timelineStartMs: 0, timelineDurationMs: 100 } })
