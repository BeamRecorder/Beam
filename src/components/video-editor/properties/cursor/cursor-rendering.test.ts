...
describe('cursor rendering', () => {
  ...
  it('anchors the cursor to Safari content below the toolbar', () => {
    ...
    expect(cursorPositionAt(cursor(.5, .5), { width: 400, height: 200 }, rect, false, undefined, false, false, {
      frame: 'safari', frameShowMenu: true, frameShowScrollbars: true, frameChromeScale: 1,
    })).toEqual({ x: content.x + content.width / 2, y: content.y + content.height / 2 })
  })

  it('includes Windows 95 chrome options and scale in cursor placement', () => {
    const rect = { x: 0, y: 0, width: 400, height: 260 }
    const appearance = { frame: 'windows-95' as const, frameShowMenu: true, frameShowScrollbars: false, frameChromeScale: 1.5 }
    ...
    const position = cursorPositionAt(cursor(.25, .75), { width: 400, height: 260 }, rect, false, undefined, false, false, appearance)
    expect(position).toEqual({ x: content.x + content.width * .25, y: content.y + content.height * .75 })
  ...
  it('maps cropped source coordinates into the decorated content', () => {
    ...
    const content = frameContentRect(rect, 'safari')
    expect(cursorPositionAt(cursor(.5, .5), { width: 800, height: 400 }, rect, false, undefined, false, false, {
      frame: 'safari', frameShowMenu: true, frameShowScrollbars: true, frameChromeScale: 1,
    }, { x: .25, y: .25, width: .5, height: .5 })).toEqual({ x: content.x + content.width / 2, y: content.y + content.height / 2 })
  })
