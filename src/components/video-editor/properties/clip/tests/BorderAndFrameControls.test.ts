...
const Switch = {
  ...
  template:
    '<button class="switch-stub" @click="$emit(\'update:modelValue\', !modelValue)">switch</button>',
};
const ColorPicker = {
  emits: ["update:modelValue"],
  template:
    "<button class=\"color-stub\" @click=\"$emit('update:modelValue', '#123456')\">color</button>",
};
const BigSlider = {
  emits: ["update:modelValue"],
  template:
    '<button class="slider-stub" @click="$emit(\'update:modelValue\', 12)">slider</button>',
};
const Input = {
  props: ["modelValue"],
  emits: ["update:modelValue"],
  template:
    '<input id="frame-title" :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" />',
};
