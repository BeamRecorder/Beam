...
const Switch = {
  props: ['modelValue'],
  emits: ['update:modelValue'],
  template: '<button class="enabled-switch" @click="$emit(\'update:modelValue\', !modelValue)">Enabled</button>',
}
const Button = { emits: ['click'], template: '<button class="delete-button" @click="$emit(\'click\')"><slot /></button>' }

describe('AudioClipPropertiesPanel', () => {
  it('renders the empty state without a clip', () => {
    const wrapper = mount(AudioClipPropertiesPanel, { props: { clip: null }, global: { stubs: { BigSlider, Switch, Button } } })
