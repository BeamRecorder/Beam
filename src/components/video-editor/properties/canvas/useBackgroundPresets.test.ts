...
const preferences = (overrides: Record<string, unknown> = {}): PreferenceSettings => ({
  schemaVersion: 2, theme: 'light', recordingBar: { visibility: 'always' }, devices: {}, shortcuts: {},
  backgroundPresets: { colors: ['#abcdef'], gradients: [{ type: 'radial', angle: 30, stops: [{ id: 'a', position: 0, color: '#000000', alpha: 1 }, { id: 'b', position: 1, color: '#ffffff', alpha: 1 }] }] },
...
describe('background presets', () => {
  it('hydrates saved presets, applies valid built-in overrides, and unsubscribes on disposal', async () => {
    ...
    capture.getPreferences.mockResolvedValue(preferences({ 'color:#111827': '#123456', 'gradient:violet': { type: 'radial', angle: 20, stops: [{ id: 'x', position: 0, color: '#000000', alpha: 1 }, { id: 'y', position: 1, color: '#ffffff', alpha: 1 }] } }))
  ...
  it('edits a built-in color live, selects the changed value, and persists its override', async () => {
    ...
    expect(capture.updatePreferences).toHaveBeenCalledWith(expect.objectContaining({
      backgroundPresets: expect.objectContaining({ colors: ['#abcdef'] }),
      extras: expect.objectContaining({ backgroundPresetOverrides: expect.objectContaining({ [violet.id]: '#aabbcc' }) }),
    }))
  ...
  it('renames and persists an edited custom color without adding a duplicate', async () => {
    ...
    expect(capture.updatePreferences).toHaveBeenLastCalledWith(expect.objectContaining({ backgroundPresets: expect.objectContaining({ colors: ['#fedcba'] }) }))
  ...
  it('updates and persists custom gradients by index, while cloning the caller value', async () => {
    ...
    const gradient = { type: 'linear' as const, angle: 450, stops: [{ id: 'start', position: 0, color: '#111111', alpha: 1 }, { id: 'end', position: 1, color: '#222222', alpha: 1 }] }
    ...
    expect(capture.updatePreferences).toHaveBeenLastCalledWith(expect.objectContaining({ backgroundPresets: expect.objectContaining({ gradients: [expect.objectContaining({ angle: 90 })] }) }))
  })
