...
vi.mock('./useVideoPlayer', async () => {
  ...
  return { useVideoPlayer: () => {
    const player = {
      currentTime: ref(0), isPlaying: ref(false), duration: ref(0), volume: ref(100), videoSrc: ref<string | null>(null),
      importedBackgrounds: ref([]), selectedBackground: ref(null), selectedBackgroundMedia: ref(null), backgroundBlurPercent: ref(0), backgroundGroups: ref([]),
...
vi.mock('./useProjectZoom', async () => {
  const { ref } = await import('vue')
  return { useProjectZoom: () => {
    const value = { zoomElements: ref([]), generatedSessions: ref([]), ensureAutomaticZooms: vi.fn() }
    state.zoomState = value
    return value
  } }
})
vi.mock('./useProjectEditorState', () => ({ useProjectEditorState: () => {
  const value = { load: vi.fn().mockResolvedValue(undefined), scheduleSave: vi.fn() }
  state.editorState = value
  return value
} }))
vi.mock('../properties/cursor/useCursorReplacer', async () => {
  const { ref } = await import('vue')
  return { useCursorReplacer: () => {
    const value = {
      selectedCursor: ref('automatic'), cursorSize: ref(24), cursorColor: ref('#ffffff'), enableShadow: ref(true), shadowBlur: ref(8), shadowColor: ref('#000000'), shadowDirection: ref('bottom-right'),
...
vi.mock('../../export/composition/snapshot', () => ({ createCompositionSnapshot: (...args: unknown[]) => { state.createCompositionSnapshot(...args); return { snapshot: true } } }))
vi.mock('../composition/engine/clip-engine', () => ({ compositionDurationMs: (...args: unknown[]) => { state.compositionDurationMs(...args); return 2000 } }))
...
describe('useVideoEditor', () => {
  ...
  it('initializes dependencies, synchronizes source/project changes and builds export data', async () => {
    ...
    const Harness = defineComponent({ setup: () => (api = useVideoEditor({ videoSrc, project: projectRef, editorData }), {}), template: '<div />' })
  ...
  it('refreshes the background library from the native subscription and tolerates failures', async () => {
    let refresh!: () => void
    capture.listBackgroundLibrary.mockRejectedValueOnce(new Error('library unavailable')).mockResolvedValueOnce([{ id: 'later' }])
    capture.onBackgroundLibraryChanged.mockImplementation((listener) => { refresh = listener; return vi.fn() })
    const videoSrc = ref<string | null>(null)
    const projectRef = ref(null)
    const editorData = ref(null)
    const Harness = defineComponent({ setup: () => useVideoEditor({ videoSrc, project: projectRef, editorData }), template: '<div />' })
    const wrapper = mount(Harness)
    await flushPromises()
    refresh()
    await flushPromises()
