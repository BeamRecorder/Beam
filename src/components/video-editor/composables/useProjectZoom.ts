...
export function useProjectZoom(options: {
  ...
  const ensureAutomaticZooms = () => {
    ...
    if (!sessionId || generatedSessions.value.some((record) => record.sessionId === sessionId && record.algorithmVersion >= ZOOM_ALGORITHM_VERSION)) return;
    generateZooms(false);
  };
  ...
  const previewMoveZoom = (id: string, startMs: number, endMs: number) => {
    zoomElements.value = zoomElements.value.map((element) => element.id === id ? { ...element, startMs, endMs } : element);
  };
