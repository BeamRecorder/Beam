...
const makeEditorData = () =>
  ({ tracks: [{ kind: "screen", format: { frameRate: 60 } }] }) as any;
...
describe("useVideoEditor", () => {
  ...
  it("initializes dependencies, synchronizes source/project changes and builds export data", async () => {
    ...
    const Harness = defineComponent({
      setup: () => (
        (api = useVideoEditor({ videoSrc, project: projectRef, editorData })),
        {}
      ),
      template: "<div />",
    });
    ...
    expect(state.player.setUserBackgrounds).toHaveBeenCalledWith([
      { id: "background-1" },
    ]);
  ...
  it("refreshes the background library from the native subscription and tolerates failures", async () => {
    ...
    const Harness = defineComponent({
      setup: () =>
        useVideoEditor({ videoSrc, project: projectRef, editorData }),
      template: "<div />",
    });
    const wrapper = mount(Harness);
    await flushPromises();
