...
export function useClipComposition(options: {
  ...
  const videoHasAudio = async (asset: MediaAsset) => {
    ...
    try {
      ...
      input = new Input({ source: new BlobSource(await response.blob()), formats: ALL_FORMATS });
  ...
  const addElementAt = async (kind: "video" | "image" | "sound" | "caption", requestedStartMs: number) => {
  ...
  const addElement = (kind: "video" | "image" | "sound" | "caption") =>
    addElementAt(kind, options.currentTimeSec.value * 1_000);
  const addCaptionAtTime = (startMs: number) => addElementAt("caption", startMs);

  const updateCaption = (caption: CaptionClip) => {
    composition.value = updateClip(composition.value, caption.id, () => clone(caption));
  };

  const trimClipEdge = (clipId: string, edge: "start" | "end", timeMs: number) => {
    composition.value = trimClip(composition.value, clipId, edge, Math.round(timeMs));
  };
  const previewClipEdge = trimClipEdge;
  const moveClipTo = (clipId: string, startMs: number) => {
    composition.value = moveClip(composition.value, clipId, startMs);
  };
  const previewMoveClip = moveClipTo;
