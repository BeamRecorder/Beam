...
const button = (time: number, x = .86, y = .84): CursorEvent => ({ event: "button", sessionNs: time * 1_000_000_000, button: 1, pressed: true, normalizedX: x, normalizedY: y });
...
describe("cursor motion", () => {
  ...
  it("keeps start, end and click positions as anchors", () => {
    const anchors = extractCursorMotionAnchors(events(move(0, 0, 0), move(1, .3, .2), move(2, 1, 1), button(1.8, .91, .17)));
    expect(anchors[0]).toMatchObject({ kind: "start", timeSeconds: 0, x: 0, y: 0 });
    const clickAnchor = anchors.find((anchor) => anchor.kind === "click");
    expect(clickAnchor?.timeSeconds).toBe(1.8);
    expect(clickAnchor).toMatchObject({ x: .91, y: .17 });
    expect(anchors.at(-1)).toMatchObject({ kind: "end", timeSeconds: 2, x: 1, y: 1 });
  });
  ...
  it("enforces the minimum travel time for a short movement", () => {
    const settings = createDefaultCursorMotionSettings();
    const timeline = createCursorMotionTimeline(events(move(0, 0, 0), move(.2, .1, .05)), settings);
    expect(timeline.segments[0].endSeconds - timeline.segments[0].startSeconds).toBeGreaterThanOrEqual(.18);
  });

  it("arrives at a click deadline even when the recorded move is fast", () => {
    const settings = createDefaultCursorMotionSettings();
    const timeline = createCursorMotionTimeline(events(move(0, 0, 0), move(.06, 1, 0), button(.06, 1, 0)), settings);
    expect(timeline.targetAt(.06)).toEqual({ x: 1, y: 0 });
    expect(timeline.segments.at(-1)?.endSeconds).toBe(.06);
  });
  ...
  it("keeps the spring within normalized bounds and resets after a seek", () => {
    ...
    const raw = { x: 0, y: 0, visible: true, cursorId: null, shapeId: null, cursorKind: null, hotspot: { x: 0, y: 0 } };
    player.sample(0, raw);
    const forward = player.sample(.2, { ...raw, x: 1 });
    const seek = player.sample(.05, { ...raw, x: 0 });
    expect(forward?.x).toBeGreaterThanOrEqual(0);
    expect(seek?.x).toBeCloseTo(player.timeline.targetAt(.05)?.x ?? 0);
  });

  it("selects a larger directional blur kernel as speed increases", () => {
    const slow = motionBlurTrail({ x: .1, y: .1 }, { x: .09, y: .1 }, .1, .4, { width: 1920, height: 1080 });
    const fast = motionBlurTrail({ x: .9, y: .1 }, { x: .1, y: .1 }, 1 / 30, .4, { width: 1920, height: 1080 });
