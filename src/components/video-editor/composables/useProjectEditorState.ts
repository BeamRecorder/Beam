...
export function useProjectEditorState(options: {
  ...
  const snapshot = (): ProjectEditorState => ({
    ...
    presentation: {
      ...
      background: options.selectedBackground.value && !["image", "video"].includes(options.selectedBackground.value.kind)
  ...
  const load = async (projectId: string) => {
    ...
    try {
      ...
      options.selectedBackground.value = normalizeBackgroundValue(state.presentation.background)
