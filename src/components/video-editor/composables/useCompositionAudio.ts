...
const SEEK_TOLERANCE_SECONDS = .08;
...
export function useCompositionAudio(input: {
  ...
  const assetFor = (clip: AudioClip) => input.composition.value.assets.find((asset) => asset.id === clip.assetId) ?? null;
  ...
  const mixSignature = () => [
    input.volume.value,
    ...audioClips.value.map((clip) => `${clip.id}:${clip.assetId}:${clip.timelineStartMs}:${clip.timelineDurationMs}:${clip.sourceInMs}:${clip.sourceDurationMs}:${clip.playbackRate}:${clip.enabled}:${clip.volume}`),
    ...
    `active:${activeClipsAt(input.composition.value, input.currentTime.value * 1_000).filter(isAudioClip).map((clip) => clip.id).join(",")}`,
  ].join("|");
  ...
  const startAt = async (timelineSeconds: number, requestedSignature = mixSignature()) => {
    ...
    masterGain?.gain.setTargetAtTime(Math.max(0, Math.min(1, input.volume.value / 100)), now, .004);
  ...
  const synchronize = () => {
    ...
    if (Math.abs(input.currentTime.value - expected) > SEEK_TOLERANCE_SECONDS) void startAt(input.currentTime.value, signature);
  };

  watch(
    () => input.composition.value.assets.filter((asset) => audioClips.value.some((clip) => clip.assetId === asset.id)).map(audioAssetSignature).join("|"),
    preload,
    { immediate: true },
  );
  watch([input.currentTime, input.isPlaying, input.volume, audioClips], synchronize, { immediate: true, deep: true, flush: "post" });
  onBeforeUnmount(() => {
    generation += 1;
    stopScheduled();
