  ...
  return {
    useVideoEditor: vi.fn(() => {
      ...
      const composition = ref<ClipComposition>({
        schemaVersion: 1,
        assets: [
          { id: "screen-asset", kind: "video", name: "Screen", fileName: "screen.mp4", durationMs: 2_000, width: 1_280, height: 720, src: "screen.mp4", origin: "session" },
          { id: "audio", kind: "audio", name: "Audio", fileName: "audio.wav", durationMs: 2_000, width: null, height: null, src: "audio.wav", origin: "project" },
        ],
        clips: [
          { id: "screen", kind: "screen", name: "Screen", assetId: "screen-asset", timelineStartMs: 0, timelineDurationMs: 2_000, sourceInMs: 0, sourceDurationMs: 2_000, playbackRate: 1, enabled: true, order: 0, transform: { x: 0, y: 0, width: 1, height: 1 } },
          { id: "audio", kind: "audio", name: "Audio", assetId: "audio", role: "system", timelineStartMs: 0, timelineDurationMs: 2_000, sourceInMs: 0, sourceDurationMs: 2_000, playbackRate: 1, enabled: true, order: 1, volume: 100 },
        ],
      } as ClipComposition);
      const selectedClipId = ref<string | null>(null);
      const selectedClip = computed(() => composition.value.clips.find((clip) => clip.id === selectedClipId.value) ?? null);
      const player = {
        isPlaying: ref(false), currentTime: ref(0), duration: ref(2), volume: ref(100), videoSrc: ref(""),
        selectedBackground: ref(null), selectedBackgroundMedia: ref(null), backgroundBlurPercent: ref(0),
        backgroundGroups: ref([]), addBackground: vi.fn(),
      };
      const cursor = {
      ...
      const compositionState = {
        ...
        isSystemAudioEnabled: ref(true), isMicAudioEnabled: ref(true), selectClip: vi.fn((id: string) => { selectedClipId.value = id; }),
        addElement: vi.fn().mockResolvedValue(undefined), addCaptionAtTime: vi.fn(), updateCaption: vi.fn(),
        trimClipEdge: vi.fn(), moveClipTo: vi.fn(), splitSelectedClip: vi.fn(), deleteSelectedClip: vi.fn(),
        reorderVisualClip: vi.fn(), updateSelectedAppearance: vi.fn(), updateSelectedTransform: vi.fn(),
        previewSelectedTransform: vi.fn(), updateSelectedCrop: vi.fn(), updateSelectedMirrored: vi.fn(),
        updateSelectedRate: vi.fn(), updateSelectedVolume: vi.fn(), updateSelectedEnabled: vi.fn(),
        toggleClip: vi.fn(), detachSelectedClip: vi.fn(),
      };
      const zoomState = {
        zoomElements: ref([]), selectedZoomId: ref<string | null>(null), selectedZoom: ref(null),
        canGenerateZooms: ref(true), hasAutomaticZooms: ref(false), addZoomAtTime: vi.fn(),
        generateZooms: vi.fn(), updateZoom: vi.fn(), trimZoomEdge: vi.fn(), moveZoom: vi.fn(),
        previewZoom: vi.fn(), deleteSelectedZoom: vi.fn(),
      };
...
vi.mock("../composables/useEditorUndoRedo", async () => {
  ...
  return { useEditorUndoRedo: vi.fn(() => ({
    recordSnapshot: vi.fn(), undo: vi.fn(), redo: vi.fn(), canUndo: ref(false), canRedo: ref(false), lastAction: ref(null),
  })) };
});

vi.mock("../../export/useExportJob", async () => {
  const { ref } = await import("vue");
  return { useExportJob: vi.fn(() => {
...
vi.mock("../Topbar.vue", async () => {
  ...
  return { default: defineComponent({
    name: "MockTopbar", emits: ["back-to-hud", "open-project", "undo", "redo"],
    setup(_, { emit }) { return () => h("div", { class: "mock-topbar" }, [
...
vi.mock("../sidebar/SidebarPanel.vue", async () => {
  const { defineComponent, h } = await import("vue");
  return { default: defineComponent({ name: "MockSidebar", emits: ["select-tab"], setup(_, { emit }) { return () => h("button", { class: "sidebar-tab", onClick: () => emit("select-tab", "zoom") }); } }) };
});

vi.mock("../properties/PropertiesPanel.vue", async () => {
  const { defineComponent, h } = await import("vue");
  return { default: defineComponent({
    name: "MockProperties", emits: ["update:system-volume", "update:mic-volume", "import:background", "update:selected-background", "update:canvas", "update:zoom", "delete:zoom", "generate:zooms", "update:caption", "update:composition", "select-caption", "delete-clip", "split-clip", "update:clip-rate", "update:clip-volume", "update:clip-enabled", "unlink-clip", "update:clip-is-mirrored", "update:clip-appearance", "update:clip-transform", "reset:clip-transform"],
    setup(_, { emit }) { return () => h("div", { class: "mock-properties" }, [
      ...
      h("button", { class: "import-background", onClick: () => emit("import:background", { kind: "color", color: "#f00" }) }),
      h("button", { class: "update-canvas", onClick: () => emit("update:canvas", { preset: "1:1", width: 1080, height: 1080, showBackground: true }) }),
      ...
      h("button", { class: "appearance", onClick: () => emit("update:clip-appearance", { borderEnabled: true }) }),
...
vi.mock("../canvas/EditorCanvas.vue", async () => {
  const { defineComponent, h } = await import("vue");
  return { default: defineComponent({
    name: "MockEditorCanvas", emits: ["update:zoom", "preview:zoom", "select:clip", "select:canvas", "deselect:transform-clip", "update:clip-transform", "preview:clip-transform", "update:clip-crop", "done:crop", "deselect:zoom", "update:is-playing", "update:current-time"],
    setup(_, { emit }) { return () => h("div", { class: "mock-canvas" }, [
      h("button", { class: "select-audio", onClick: () => emit("select:clip", "audio") }),
      h("button", { class: "select-canvas", onClick: () => emit("select:canvas") }),
      h("button", { class: "update-zoom", onClick: () => emit("update:zoom", { id: "z", sessionId: "s", startMs: 0, endMs: 1000, focus: { cx: .5, cy: .5 }, depth: 2, mode: "manual" }) }),
      h("button", { class: "preview-zoom", onClick: () => emit("preview:zoom", { id: "z" }) }),
      h("button", { class: "transform", onClick: () => emit("update:clip-transform", { x: .1, y: .1, width: .5, height: .5 }) }),
      h("button", { class: "crop", onClick: () => emit("update:clip-crop", { x: 0, y: 0, width: 1, height: 1 }) }),
...
vi.mock("../canvas/CanvasToolbar.vue", async () => {
  const { defineComponent, h } = await import("vue");
  return { default: defineComponent({ name: "MockCanvasToolbar", emits: ["select:preset", "toggle:crop"], setup(_, { emit }) { return () => h("div", [
...
vi.mock("../timeline/TimelineToolbar.vue", async () => {
  const { defineComponent, h } = await import("vue");
  return { default: defineComponent({ name: "MockTimelineToolbar", emits: ["update:is-playing", "update:current-time", "add:element"], setup(_, { emit }) { return () => h("div", [
...
vi.mock("../timeline/EditorTimeline.vue", async () => {
  const { defineComponent, h } = await import("vue");
  return { default: defineComponent({ name: "MockEditorTimeline", emits: ["select:zoom", "select:clip", "toggle:clip", "trim:clip", "move:clip", "trim:zoom", "move:zoom", "add:zoom", "add:caption", "reorder:clip"], setup(_, { emit }) { return () => h("div", [
...
describe("VideoEditor", () => {
  ...
  it("updates role volumes and protects editable fields from destructive keyboard shortcuts", async () => {
    ...
    expect(editorState.store.compositionState.composition.value.clips.find((clip: any) => clip.role === "system")?.volume).toBe(150);
  });
});
