...
<template>
    <div class="gradient-editor">
        ...
        <div class="gradient-visual-container">
            ...
            <div ref="trackRef" class="gradient-track" :class="{ 'is-locked': stops.length >= effectiveMaxStops }"
                ...
                <!-- Interaction markers -->
                <div v-for="stop in stops" :key="stop.id" class="stop-handle" :class="{
                    active: selectedStopId === stop.id,
                    dragging: draggingStopId === stop.id,
                    'over-trash': draggingStopId === stop.id && isOverTrash,
                }" :style="{ left: `${stop.position * 100}%` }" @pointerdown="startDragging($event, stop.id)"
