...
describe('useGradient', () => {
  ...
  const mountComposable = (overrides: Record<string, unknown> = {}) => {
    ...
    return { props, emit, get state() { return state; } };
  };
  ...
  it('normalizes malformed values, clamps fields, and builds the preview style', () => {
    ...
    expect(mounted.state.gradientPreviewStyle.value.background).toContain(
  ...
  it('falls back to two default stops and merges custom presets without duplicates', () => {
    ...
    expect(mounted.state.allPresets.value.slice(0, 3).map((preset) => preset.id)).toEqual([
      'Fire',
      'Custom',
      'Invalid',
    ]);
  ...
  it('updates gradient settings and selected stop values with clamping', async () => {
    ...
    expect(mounted.emit).toHaveBeenLastCalledWith(
      'update:modelValue',
      expect.objectContaining({ angle: 360 }),
    );

    await mounted.state.handleStopClick(
    ...
    expect(mounted.state.selectedStop.value).toEqual(
      expect.objectContaining({ alpha: 1, position: 0 }),
    );
