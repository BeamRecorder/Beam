...
describe("ShortcutInput", () => {
  ...
  it.each([
    ...
    const wrapper = mount(ShortcutInput, { props: { modelValue: "" } });
    await wrapper.get(".shortcut-input").trigger("click");
    const modifier = code.startsWith("Shift") ? { shiftKey: true } : code.startsWith("Alt") ? { altKey: true } : code.startsWith("Meta") ? { metaKey: true } : {};
  ...
  it("does not keep a Tab recording active and combines all modifier types", async () => {
    ...
    await wrapper.get(".shortcut-input").trigger("click");
    await wrapper.get(".shortcut-input").trigger("keydown", key("KeyA", { key: "a", ctrlKey: true, altKey: true, shiftKey: true, metaKey: true }));
