...
const fetchFor = (files) => async (url, options = {}) => {
  if (url.includes('/tree/')) return new Response(JSON.stringify([...REQUIRED_PATHS].map((file) => ({ type: 'file', path: file, size: files[file].length, lfs: { oid: crypto.createHash('sha256').update(files[file]).digest('hex') } }))), { status: 200 })
