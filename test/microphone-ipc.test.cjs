...
function sessionFixture() {
  ...
  fs.writeFileSync(
    manifestPath,
    JSON.stringify({
      selectedSources: { screen: 'display:1', microphone: null },
...
test('finalizes ordered Opus WebM chunks and merges a completed microphone track', () => {
  ...
  assert.deepEqual(manifest.tracks[0].segments[0], {
    ...manifest.tracks[0].segments[0],
    path: 'microphone/segment-0001.webm',
    startNs: 100,
    endNs: 200,
    complete: true,
  })
  assert.deepEqual(
    fs.readFileSync(path.join(session.directory, 'microphone', 'segment-0001.webm')),
    Buffer.from([1, 2, 3, 4]),
  )
})
...
test('rejects malformed sources, cross-renderer writes, invalid formats and out-of-order chunks', () => {
  ...
  storage.registerSession(session)
  assert.throws(
    () => storage.begin(23, { sessionId, sourceId: 'microphone:1', format, startNs: 0 }),
    /Invalid Chromium microphone source/,
  )
  assert.throws(
    () => storage.begin(23, { sessionId, sourceId, format: { ...format, codec: 'aac' }, startNs: 0 }),
    /Invalid microphone format/,
  )
  const opened = storage.begin(23, { sessionId, sourceId, format, startNs: 0 })
  assert.throws(
    () => storage.write(24, { jobId: opened.jobId, sequence: 0, data: new Uint8Array([1]) }),
    /not authorized/,
  )
