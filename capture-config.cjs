...
function selectSource(sources, kind, requestedId) {
  ...
  return sources.find((source) => source.kind === kind && source.isDefault) ||
    sources.find((source) => source.kind === kind) || null
}
...
function buildDefaultCaptureConfig(catalog, options, environment) {
  ...
  const microphone = options.microphoneId === null
    ? null
    : selectSource(sources, 'microphone', options.microphoneId)
  const camera = options.cameraId === null
    ? null
    : selectSource(sources, 'camera', options.cameraId)
  return {
    projectId: options.projectId || randomUUID(),
    screen: { mode: 'source', sourceId: screen.id },
    systemAudio: options.systemAudio && capabilities.systemAudio
      ? { mode: environment.platform === 'darwin' ? 'screen-capture-mix' : 'default-mix' }
      : null,
    ...
    camera: camera ? {
      sourceId: camera.id,
      preferredWidth: positiveInteger(options.cameraWidth, 1280, 'cameraWidth'),
      preferredHeight: positiveInteger(options.cameraHeight, 720, 'cameraHeight'),
      preferredFps: positiveInteger(options.cameraFps, 30, 'cameraFps'),
      preferredPixelFormat: 'mjpeg',
    } : null,
    cursor: options.cursor !== false && capabilities.separateCursor
      ? {
          mode: 'separate',
